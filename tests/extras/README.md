...
> **番号だけで引用しないこと。** 本 README には「隔離・後始末の規約」8 / 9 が既にあり、
> コード内の引用（`ta-26` / `ta-45` 等 30 箇所超）はすべてそちらを指す。本節の規則は
> 衝突を避けるため **`P-` 接頭辞つきの ID** を正本とし、引用時は `P-8` のように書く
> （「規約 8」は常に「隔離・後始末の規約」8 を意味する / #1250 m-1）。

1. **[P-1] rc と一意 reason トークンの対で判定する。**
...
2. **[P-2] 選言（`A|B`）のみの述語を PASS 条件にしない。** `grep -qiE 'SKIP|PASS'` の
   ように「どちらかが出ていればよい」と書いた時点で、その TC は分岐を
   区別していない。分岐ごとに期待 rc と期待トークンを固定する。
3. **[P-3] reason トークンは分岐固有にする。** 対象コードに一意な語が無い場合は
...
4. **[P-4] 対照（negative control）を同じ TC に置く。** 「期待の入力で PASS」だけでは
...
5. **[P-5] 文字列の存在ではなく構造を見る。** 「ファイルのどこかに X がある」は
   ほぼ常に弱すぎる。位置・ブロック・同一行といった**構造**で書く
   （例: `ta-70` TC-01 は「先頭行 / ガード開始行 / 閉じ行を厳密照合し、
   その間の中身を**正典テンプレート 1 本とバイト列で完全一致**させる」。
   **手書きの字句 allowlist は #1250 R4 で 2 度目の回避クラスが出たため
   採らない** — 部分実装の字句器は未実装構文がそのまま穴になる）。
6. **[P-6] ループで集めた違反リストの空判定は、母数の floor と連言にする。**
...
7. **[P-7] 新規・是正した TC は変異注入で検出力を実証する。** 守りたい不変条件を
   破る変異を入れ、**是正前は SURVIVE し是正後は KILL される**ことを実測で
   示す（#1163 / #1176 で確立した順序）。変異が実際に入ったことを先に
   確認する（`diff` / `grep -c` が 0 行なら変異は空振りしている）。
   **変異は検査対象の領域内に入れること**。領域外を壊した変異は「検出しない」が
   正しい答えになり、検出力の実証にならない（2026-09-07 実測: patch 文書の
   `<!-- PG-PATCH-BEGIN -->` ブロックの**外**にある散文を変異させ、「抽出結果に
   差が出ない」を一見 control 失敗と読み違えた）。なお `P-4` の対照は**入力**を
   変えて落ちることを見る規則であり、本項の**対象そのものを壊す**変異とは別物。
8. **[P-8] 緩和フォールバックを無制限に置かない。** 「判別できない入力は旧来の
...
9. **[P-9] 規約の正本が例示コードなら、その例示自身を検査器に通す。** 例示を
   fixture として抽出して検査器へ流し、**抽出形（shape）まで連言で要求**する
   （返り値だけを見ると、例示が緩和フォールバックへ落ちても緑になる /
   #1250 F2。`ta-26` TC-38 が実装例）。
10. **[P-10] 期待値が環境依存なら、定数を捨てるのではなく実行時に測る。**
    rc が実行系に依存する場合（例: 監査ログへの**リダイレクト**書込失敗時の rc は
    `/bin/sh` が bash なら 1、dash なら 2。**`mkdir` 失敗は両方 1**）、
    `P-1` を守るために期待 rc を**捨ててはならない**。
    捨てると `P-2` の選言になり、**その rc の差そのものが検査したい不変条件**
    だったときに検査が消える。代わりに **probe で実行時に測って定数の代わりに
    する**。probe には次を課す（`ta-79` の `_t79_probe_rc` が実装例 / #1278）:
    - **[適用条件] probe を使ってよいのは、実行系（`/bin/sh` の実体等）×
      エラーモードの実測表で環境依存性を示せた TC に限る。** 示せないなら
      `P-1` どおり定数を維持する。**本項は `P-1` / `P-2` の一般的な緩和条項
      ではない**（実例: `ta-79` の TC-10 / TC-11 は「patch 適用後の rc は
      shell を問わず同一」を実測したうえで定数据え置き）
    - **[適用条件] そもそも env 依存の前提を作らない設計が上位**。作らずに済むなら
      定数のままでよい（実例: `ta-80` は監査ログを書けない状態を一切作らないため
      観測 rc がすべて hook 自身の exit になり、rc + トークン対を据え置いている）
    - **probe 断片は被検査対象の失敗箇所と同じ構文形・同じ `set -e` 到達文脈で書く** —
      形がずれると、本体側が変わったとき probe 由来の期待 rc が本体と乖離し、
      TC が**無関係な理由で恒常 FAIL** になる（`ta-79` は「断片は hook の
      `log_event` と同じ形にする」を明記して同形を保っている）
    - **被検査対象と同じ実行系で起動する** — hook の配線が `sh <script>` なら
      probe も `PATH` 解決の `sh` で測る。`$SHELL` や実行中シェルで測ると
      別実体の値を導出してしまう
    - **エラーモードごとに別々に測る** — 同じ関数の失敗でも、リダイレクト失敗と
      `mkdir` 失敗で rc が違う（前者だけ dash で 2、後者は両方 1）
    - **probe が期待値を導出できなかったら採用せず、その TC を FAIL / SKIP にする** —
      probe が 0 を返したまま採用すると以降の連言が**恒偽条件**になり、TC が常に
      落ちるか期待値ごと無効化される
    - **probe を被検査対象自身から導出しない**（恒真になる）。あわせて「probe が
      非 0 を返す」（前提が再現できている）と「書き込み可能な対象では 0 を返す」
      （probe 自体が壊れていない）の両方を `P-4` の対照として置く

### 本節の残存脅威モデル（機械検証の射程 / #1250 F5）

本節の規則のうち **機械検証を持つのは `P-5` / `P-8` / `P-9` の 3 つだけ**である
（`P-6` は見送り。以前ここに「規則 5・6・8・9」と書いていたのは直下の表と
矛盾する誤記だった / #1250 m-2）。内訳と、機械化を見送った判断の根拠を残す。

**表の読み方（#1250 m-5）**: 「機械検証」欄の TC が検証しているのは
**その規則に従って書かれた特定の実装が壊れていないこと**であって、
**規則そのものが README に存在すること**でも、**他の TC がその規則を守って
いること**でもない。つまり `P-5` / `P-8` / `P-9` も *repo 全体への強制力*は
持たない — 新しい TC が `P-5` に違反しても、下表のどの TC も落ちない。

| 規則 | 機械検証（= 何が壊れたら落ちるか） |
|---|---|
| `P-1`〜`P-4`（rc + 一意トークン / 選言禁止 / 分岐固有トークン / 対照）| **なし**（レビュー時の目視） |
| `P-5`（文字列でなく構造を見る）| `ta-70` TC-01 + TC-07 — **`ta-70` の guard 正典一致判定**が緩んだら落ちる（TC-07 が **9 クラス**の変異注入対照を持つ）。他ファイルの `P-5` 違反は検出しない |
| `P-6`（違反リストの空判定に母数 floor）| **なし（見送り）**。下記参照 |
| `P-7`（変異注入で検出力を実証）| **なし**（PR 記述と実測ログで担保） |
| `P-8`（緩和フォールバックの allowlist 化）| `ta-26` TC-33 — **`ta-26` の `_T26_NOCOND_ALLOW33` 運用**が緩んだら落ちる（allowlist 外の `nocond` は FAIL）。他ファイルのフォールバックは検出しない |
| `P-9`（規約の例示自身を検査器に通す）| `ta-26` TC-38 — **README「隔離・後始末の規約」8 の例示**が検査器を通らなくなったら落ちる（shape=`cond` を連言で要求）。他の規約の例示は対象外 |
| `P-10`（環境依存の期待値は実行時 probe で測る） | **なし**（レビュー時の目視）。probe 自体の健全性は `P-4` の対照として各 TC が持つ（`ta-79` の `TC-12/probe-1` / `probe-2`） |

**`P-6` の静的検出を見送った根拠（実測）**: 「`[ -z "$violations" ]` 相当の
...
**この結論の射程（#1250 m-4）**: 上で測ったのは **「1 行を正規表現で見る静的
検出器」の 2 変種だけ**である。したがって言えるのは
**「`P-6` は静的な行単位の検出では実用にならない」**までであって、
**「`P-6` の機械化はいかなる手段でも不可能」ではない**。実装していない
（＝未評価の）方向が少なくとも 1 つある: **走査母数を強制的に空にする
test-only knob を入れ、ランタイムに「母数 0 でも緑になる TC」を炙り出す**
（静的解析ではなく実行時の vacuous 検出）。本 PBI ではこれを**実装しない**。
評価していないだけであり、否定していない。

したがって **`P-6` は当面レビュー時の目視で担保する**。
これは多層防御の 1 層が欠けている状態であり、`P-6` 違反は
**C-4 Human レビューでのみ捕捉される**。

`P-1`〜`P-4` / `P-7` / `P-10` も同様に機械検証を持たない。**本節は「検査器がある規約」
ではなく「レビュー観点の正本」であり、完全性を主張しない。**
