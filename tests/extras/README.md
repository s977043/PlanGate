...
4. **[P-4] 対照（negative control）を同じ TC に置く。** 「期待の入力で PASS」だけでは
   ...
   **対照は検査対象の領域内を壊すこと**。領域外を壊した対照は「検出しない」が
   正しい答えになり、対照として機能しない（2026-09-07 実測: patch 文書の
   `<!-- PG-PATCH-BEGIN -->` ブロックの**外**にある散文を変異させた対照が
   「抽出結果に差が出ない」を返し、一見 control 失敗に見えた）。
...
10. **[P-10] 期待値が環境依存なら、定数を捨てるのではなく実行時に測る。**
    rc が実行系に依存する場合（例: 監査ログ書込失敗時の rc は `/bin/sh` が
    bash なら 1、dash なら 2）、`P-1` を守るために期待 rc を**捨ててはならない**。
