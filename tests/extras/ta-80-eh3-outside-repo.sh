...
#
# ── rc 固定と `sh` 実体依存についての点検結果（#1289 CI fail 由来）───
# ta-79 の #1278 レーンは「監査ログを書けなくする」前提を作るため、**エラー時の
# rc が `/bin/sh` の実体（どのシェルか）に依存する**。本 worktree での実測
# （`set -eu` 下の最小コマンド / uid=502・非 root）:
#
...
#
# **OS ではなく `/bin/sh` が何かで決まる**（配線は `sh <script>`）。`/bin/sh` が
# bash な Linux では Linux でも rc=1 になる。ta-79 の gap レーンで rc を数値固定
# していたことが Ubuntu CI（`sh`=dash）の FAIL を招いた。
#
# 本ファイルは **監査ログを書けない状態を一切作らない**（各 fixture は
# docs/working/_audit を書き込み可能な通常ディレクトリとして用意する）ため、
# 観測される rc はすべて **hook 自身の exit（0 / 2）** で `sh` 実体に依存しない。
# 実測: `/bin/sh`(bash 3.2.57) と dash（PATH の `sh` も dash へ差し替え）で
# 40 PASS / 0 FAIL 一致。よって本ファイルの rc + トークン対は据え置く。
# **将来 TC を足すときも、監査ログを書けなくする前提を導入するなら rc を数値で
# 固定しないこと** — ta-79 の TC-12 のように、hook と独立した probe で当該
# エラーモードの rc を実測し、それと理由トークンの対で判定する。
