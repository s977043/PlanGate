...
- 重要な設計判断は同タスクの `decision-log.jsonl`（正本・各 TASK ディレクトリ直下）に記録する。学び/再発防止は `AGENT_LEARNINGS.md`、監査ログは `_audit`、docs 配置規約は `documentation-management.md` に従う（保存先を分離し AGENTS.md に恒常ルール以外を足さない / #578）

## 前提の実測検証（#786）

> 計画が依拠する前提は、検証コマンドと実測結果で裏取りする。検証不能な前提は直下の「Questions / Unknowns」に降格する。

| 前提 | 検証コマンド | 実測結果 | 判定 |
|---|---|---|---|
| {計画が依拠する前提} | `{検証コマンド}` | {実測結果} | ✅ / ❌ / N/A |

> 凡例: ✅ = 実測が前提を支持 / ❌ = 前提崩壊（Replan Triggers に従い計画を差分改訂する）/ N/A = 検証不能（表に残さず Questions / Unknowns へ降格する）

## Questions / Unknowns（#786）

- {検証不能・未確定の前提や不明点。解消条件（誰が・何で判断するか）を添える}
...
> C-2 ゲートのレーン責務は `review-principles.md` §7-bis / `external-reviewer-interface.md` が正本。本表はそれを成果物単位の実行計画へ落とすためのもの（レーンの新設・変更は正本側で行う）。
...
> C-1 self-review の前に `plan-review-readiness-gate.md` で確認する。各項目は `pass / needs_revision / blocked` 判定に使われるため、`TBD` / `TODO` / `必要に応じて` / `適切に` のまま残さない。
