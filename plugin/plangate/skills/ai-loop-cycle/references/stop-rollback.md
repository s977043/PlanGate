...
- **merge=Human-owned 固定**（`responsibility-classes.md`）は
  一切変更しない。本書のいかなる記述も merge の自動化・省略・条件緩和を意味しない（§6 で再確認する）。
- **事後 revert 自動化 + post-merge 監視**（[`hotl-merge-entry-criteria.md`](./hotl-merge-entry-criteria.md)
...
- 本書は `orchestrator-mode.md` の AS-1〜5（親子 PBI 分解ゲート）
...
[`design-philosophy.md`](./design-philosophy.md) I-6「停止できないループはループではない」の
...
`working-context.md`「C-3 条件付き降格」§AC-9 は、
...
| in-the-loop 前提の根拠 | — | `asset-inventory.md`:40「C-3 承認ロジックは実行前承認が前提。ai-loop では C-3 を C-3'（AI 裁定ゲート）に置換する（再設計対象）」 |
...
- **機械化可否**: **機械的に実行可能**（通常の git 操作。ただしリモートへの force-push 等
  破壊的操作は `working-context.md`「Bash 連結コマンド時の
...
- **機械化可否の位置づけ（major 指摘 F9 反映）**: この操作は append-only の記録作業であり、
  `responsibility-classes.md` の Human-owned
...
| 5 | `issued_by`（AUTO_APPROVED の発行元）の真正性が未検証（署名・HMAC 等が別途必要） | `design-philosophy.md` I-3 の既知の限界としてそのまま継承。§3.2 の human_ack はあくまで「事後 reject の意思決定」の真正性を担保するものであり、`AUTO_APPROVED` 発行自体の真正性検証（[`hotl-merge-entry-criteria.md`](./hotl-merge-entry-criteria.md) 条件1）は別スコープ | R-4 派生 |
...
- **merge の自動化・省略・条件緩和**（`responsibility-classes.md`
...
- **merge 後の revert の自動 merge**（[`hotl-merge-entry-criteria.md`](./hotl-merge-entry-criteria.md)
...
- [`docs/ai/ai-loop/design-philosophy.md`](./design-philosophy.md) I-1（承認境界の不可侵）/ I-3（裁定の決定論・既知の限界）/ I-4（安全側デフォルト）/ I-6（停止できないループはループではない）
- [`docs/ai/ai-loop/arbiter-policy.md`](./arbiter-policy.md) §6 第0の承認境界 / §7 escalate 予算 / §8 安全装置
- [`docs/ai/ai-loop/hotl-merge-entry-criteria.md`](./hotl-merge-entry-criteria.md) 条件2（事後 revert 自動化・未設計）
- `asset-inventory.md` — working-context.md C-3 ロジックの not-uses 分類（AC-9 が in-the-loop 前提であることの根拠）
- `design-hotl-metrics.md` — reversal_rate との概念区別（EPIC #822 項目3。本 PR 時点で未マージのためリンク先は暫定的に不在。項目3 のマージ後に解決する）
- `working-context.md` AC-9（巻き戻し5ステップの原型）
- `responsibility-classes.md` merge=Human-owned 固定・責務4分類
- `orchestrator-mode.md` AS-1〜5（本書とスコープが異なることの確認のみ）
- `ref-integrity-scan/SKILL.md` — 適用範囲がファイルパス参照専用であることの確認
