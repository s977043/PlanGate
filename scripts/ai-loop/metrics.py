...
`run` フィールドの無い record は「legacy」として集計母数から除外し、
除外件数を必ず出力へ明示する（無言の切り捨て禁止）。
...
def _load_records(runs_dir: Path) -> tuple[list[dict[str, Any]], list[dict[str, str]]]:
    """runs_dir 配下の *.json を読み込む。

    戻り値: (正常に読めた record のリスト, skip した file の {file, reason} のリスト)。
    破損 JSON・非 dict のトップレベル値は skip する（fail-silent 禁止で理由を記録）。
    """
...
def _is_run_record(record: dict[str, Any]) -> bool:
    """record が新形式（run メタあり）かどうかを判定する。"""
    run_meta = record.get("run")
    return isinstance(run_meta, dict) and "run_id" in run_meta
...
def collect(runs_dir: Path) -> dict[str, Any]:
    ...
    仕様:
    - run グルーピング: record["run"]["run_id"] を持つものを run 単位に集約
      （round_index 昇順）
    ...
    - legacy: run フィールド無しの record は母数から除外し、除外件数を
      legacy_count として明示する
    """
    all_records, skipped = _load_records(runs_dir)

    legacy_records = [r for r in all_records if not _is_run_record(r)]
    run_records = [r for r in all_records if _is_run_record(r)]
    ...
    # legacy record も decision 別件数には反映する（総件数の可視性を保つため）。
    # ただし first-pass rate 等の run 単位指標には一切含めない。
