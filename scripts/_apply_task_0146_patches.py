...
P1_NEW = (
    ...
    "      PLANGATE_HOOK_STRICT=1 sh \"$plangate_root/scripts/hooks/check-fix-loop.sh\" \"$task_id\" increment || {\n"
    "        printf '[verify] EHS-3 BLOCK -- fix loop limit exceeded (validation_bias=strict)\\n' >&2\n"
    "        return 1\n"
    "      }\n"
    "    else\n"
...
P2_NEW = (
    ...
    "    if [ \"${PLANGATE_VALIDATION_BIAS:-normal}\" = \"strict\" ]; then\n"
    "      PLANGATE_HOOK_STRICT=1 sh \"$plangate_root/scripts/hooks/check-handoff-elements.sh\" \"$task_id\" || {\n"
    "        printf '[handoff] EHS-2 BLOCK -- handoff.md missing required sections (validation_bias=strict)\\n' >&2\n"
    "        return 1\n"
    "      }\n"
    "    else\n"
