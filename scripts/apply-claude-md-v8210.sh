...
NEW_BODY="> 最新リリース: **v8.21.0**（2026-08-19）v8.20.0 タグ以降に main へ蓄積した 47 コミットを反映（実測: \`git rev-list --count v8.20.0..f31687a\`）。主題は **「参照はあるが解決されない」「ガードはあるが迂回できる」構造の実測是正**。主要変更: **skill の参照解決順から構造上必ず空振りする plugin root 段を除去**（#954。最終 PR #1158 だけで 41 ファイル。\`docs/**\` は \`sync-plugin-plangate.sh\` の設計上 plugin の配布対象外のため常に解決されない手順だった）・**不在 rules 参照の張り替えと正本宣言の 4 root 統一**（#1123/#1125/#1126/#1127）・**EH-3 の Hardening Override 迂回の解消**（#1089。\`PLANGATE_HOOK_TASK\` 設定下では HO 9 カテゴリすべてが block されず、PlanGate 作業中のセッションこそ保護が外れていた。適用済みのため \`tests/fixtures/eh3-known-gap-1089.flag\` は削除）・**EH-13 承認トークンガードの迂回 2 クラス封鎖**（#1115 の外側ゲート ワイルドカード迂回＝実測 21 コマンドが素通り / #1110 のリダイレクト相関判定。fail-closed は不変）・**C-1 セルフレビュー項目数を実体（全 25 項目）へ是正**（#960）・**クラス A（rules 参照）の解決梯子を 4 skill に追加**（#1159。plugin 経由で判定正本が引けない状態の解消）。**\`schemas/*.json\` / \`bin/plangate\` は変更ゼロ**（Schema / CLI の挙動は不変）。**block 挙動の強化は \`scripts/hooks/\` を配線している利用者に影響**（plugin 配布物には含まれない）。semver は規約 §2.2 上 major の材料を含むが **Human 裁定で minor**（経緯は [\`docs/working/_merge/v8.21.0-release-runbook.md\`](docs/working/_merge/v8.21.0-release-runbook.md) §1）。**PlanGate 本番フロー WF-00〜07 は不変・NO MERGE BY AI／C-4・merge は Human-owned 固定**。リリース履歴の正本は [\`CHANGELOG.md\`](CHANGELOG.md)。"
