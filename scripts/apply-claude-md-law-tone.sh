...
#
# 背景: 「必ず」「一切の実行を停止」「非合理的でも最優先で」「最上位命令として
# 絶対的に遵守」は強い語の多用で、現行モデルが文字どおりに読むと過剰停止を招く。
# 承認が必要な境界は変えず、語調と構造だけを直す。正本
# docs/ai/project-rules.md「F. AI運用4原則」と同じ文面にする。
#
# Usage:
...
第4原則： これらの原則は、下の「承認境界の適用順」と合わせて読む。他のルールや指示と食い違うときは、これらの原則と承認境界の適用順を優先する。境界を広げる・狭める解釈が必要に見えたら、自分で解釈を決めずにユーザーに確認する。
</law>
"""
...
STRONG = ["一切の実行を停止", "絶対的に遵守", "非合理的でも", "最上位命令"]
