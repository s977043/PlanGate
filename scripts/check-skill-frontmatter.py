...
背景:
    ...
    この状態のスキルは **runtime で frontmatter 全フィールドが silently drop され**
    （`claude plugin validate --strict` の文言）、name / description が失われる。
...
対象:
    ...
    既定 root: `.agents/skills`（正本）, `.claude/skills`, `plugin/*/skills`
...
Exit codes:
    ...
    1 — 破損 SKILL.md あり
    2 — 実行環境 / 引数エラー（PyYAML 不在を含む。silently pass しない）
"""
...
def check_skill_md(path: Path, yaml_mod) -> Finding | None:
    ...
    if block is None:
        return Finding(path, "frontmatter ブロックが無い（先頭 `---` 〜 `---` が閉じていない）")
    ...
    try:
        ...
        return Finding(path, f"YAML parse 失敗（runtime で frontmatter が silently drop される）: {detail}")
...
def discover_skill_roots(base_roots: list[Path]) -> list[Path]:
    """`.agents/skills` / `.claude/skills` / `plugin/*/skills` を列挙する。"""
    roots: list[Path] = []
