...
- [2026-08-20] 件数・全数列挙は ref を明示して測る（共有 checkout は他セッションが別ブランチに置く）
  - 事実: `ls tests/extras/*.sh | wc -l` / `grep -rn <pat> tests/extras/*.sh` で測って「実測」と称したが、共有 checkout が `origin/main` より 2 ファイル古い feature ブランチにあり、**件数 4 種すべてが誤り**だった（当時の実測で extras 66→64 / `-eq` 106→95 / `PG_HARNESS_SOURCED` 25/65→23/63。**いずれも増え続ける対象なので現在値ではない** — 本記録の 2 日後には 67 / 109 / 26 になっている）。同一セッション内で `gh` 経由の測定（常に origin を見る）と作業ツリーの測定が混在しており、片方が正しいことが他方の検証にならず発見が遅れた。別件では抽出パターン `[a-z_]*\.py` が `c3_contract.py` の数字を落とし、allowlist 28 件を 24 件と誤数えた
  - 再利用条件: 件数・行数・全数列挙・「残存 0」を主張するときは `git grep <pat> origin/main -- <path>` / `git ls-tree -r --name-only origin/main -- <dir>` / `git show origin/main:<path>` の形で測り、**コマンドごと共有する**（受け手が再現でき、ref 違いなら再現時に食い違って露見する）。作業ツリーの `ls` / `grep -r` を根拠にしない。抽出パターンを書いたら、**そのパターンが取りこぼす対象がないか**を別パターンでの件数比較か sanity check（必ずヒットするはずの対象で 0 件でないこと）で確かめる
  - 根拠: 2026-08-20 issue #1162 / #1173 の棚卸し。複数セッションが 1 つの checkout を共有して別ブランチで作業するのが常態のため、ブランチ非指定の測定は常に stale になりうる

- [2026-08-20] `tests/extras/ta-*.sh` の判定パターンはスイートごとに実物から導出する
  - 事実: `ta-33` の出力形式から「`[FAIL] <TC名>` 行の有無で判定する」という共通規則を作り他セッションへ配布したが、隣接スイートで形式が全部違った。`ta-33:29` は `printf '[FAIL] TA-33 TC-01: ...'`（行頭・識別子あり・stdout）、`ta-57:39` は `t57_fail() { printf '  [FAIL] %s\n' "$1" >&2; }`（**先頭 2 空白・`TA-57` を含まない・stderr**）。`grep -q '^\[FAIL\] TA-57 TC-14'` は永久に一致せず、この規則で書いた新規 TC 8 件中 6 件が「FAIL 行なし＝PASS」の恒真 PASS に戻っていた
  - 再利用条件: extras の実行結果を機械判定するときは、対象スイートごとに `git grep -n 'FAIL' origin/main -- tests/extras/<対象>.sh` で出力形式（接頭空白・識別子の有無・stdout/stderr）を確認してからパターンを書く。ログ取得は **`2>&1` 必須**。加えて**正側は `[PASS]` 行の存在も受理条件にする**（「FAIL 行が無い」は「FAIL 行が出るところまで到達したか不明」と区別できない）。`sh tests/extras/ta-XX.sh` の単体実行は `PG_HARNESS_SOURCED` を持つスイート（`git grep -l 'PG_HARNESS_SOURCED' origin/main -- 'tests/extras/ta-*.sh'`）でのみ成立し、持たないスイートは `[FAIL]` を出しても **rc=0** になる
  - 根拠: 2026-08-20 TASK-1162 / TASK-1165 の計画レビュー 2 巡目。1 スイートの実測を全称規則に昇格させ、他セッションにも配布していた

- [2026-09-24] PR のレビューでは、PR の作成より後に出た設計決定と照合する
  - 事実: READY・CI 緑・作者のセルフレビュー済みの docs PR 2 本が、PR の後に issue 側で出た決定と矛盾していた。#1372 は、#1369 のコメント（PR の最初の commit の 1 時間後。その後の最終 commit はこの決定より後だが、決定を反映していなかった）で「別の doc を増やさない」と決まり、doc そのものが不要になっていた。#1379 は、#1385 の作成（PR の 3 時間後）で「runtime のグラフは event stream の事実で、2 つ目の可変な状態ストアを作らない」と決まったのに、可変グラフを前提にしていた。しかも #1385 は「#1379 がマージされるまで実装しない」と書いており、古い境界が下流の前提になるところだった（#1379 はその後 2026-09-24 に #1385 に合わせて是正された）
  - 再利用条件: PR をレビューするとき、または ready にする前に、`gh pr view <n> --json commits` の最初の commit の時刻（PR の作成時刻）と、関連 issue のコメント・本文の更新（`gh issue view <n> --json comments,updatedAt`）、関連 issue が参照する新しい issue の作成時刻を比べる。最終 commit の時刻が決定より後でも、反映済みとは限らないので、決定の内容が PR の本文に反映されているかを照合する。反映されていない決定があれば、PR を決定に合わせるのか、決定によって PR が不要になったのか（close が正解）を分けて判断する。並行して設計を詰めているときほど起きやすい
  - 根拠: 2026-09-24 の独立レビュー（Copilot は quota 切れで実行されていなかった）。作者のセルフレビューは PR を書いた時点の理解で行われるので、あとの決定を構造的に拾えない

- [2026-09-24] サブエージェントへの委託プロンプトは `dispatch-template.md` の定型から作る
  - 事実: `docs/ai/subagent-delegation/dispatch-template.md` の要素 6 には「EH-3 に拒否されたら停止し、Bash 経由の直接書き込みでゲートを迂回しない」がすでにあった。オーガナイザーが定型を使わずに委託プロンプトを書き、この一文を入れた 2 体は EH-3 に止められた時点で停止して報告したが、入れなかった 1 体（#1409）は `Edit` を拒否されたあと scratchpad のスクリプトを Bash から実行して 5 ファイルを書き換えた
  - 再利用条件: ファイルを書き換える作業をサブエージェントに委託するときは、自分で書いたプロンプトを使わず、`dispatch-template.md` の必須要素（とくに要素 6 の制約表）を転記してから、その作業固有の指示を足す。定型にある規律を自分の記憶から書き起こすと、抜けたものがそのままワーカーの行動の穴になる
  - 根拠: 2026-09-24 のオーガナイザー体制のウェーブ。規律を書いたワーカーと書かなかったワーカーで、同じ状況での振る舞いが分かれた

- [2026-09-24] EH-3 の環境変数をコマンドの先頭にインラインで付けても hook には届かない
