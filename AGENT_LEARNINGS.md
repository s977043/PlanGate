...
- [2026-09-24] PR のレビューでは、PR の最終 commit より後に出た設計決定と照合する
  - 事実: READY・CI 緑・作者のセルフレビュー済みの docs PR 2 本が、PR の後に issue 側で出た決定と矛盾していた。#1372 は、#1369 のコメント（PR の 1 時間後）で「別の doc を増やさない」と決まり、doc そのものが不要になっていた。#1379 は、#1385 の作成（PR の 3 時間後）で「runtime のグラフは event stream の事実で、2 つ目の可変な状態ストアを作らない」と決まったのに、可変グラフを前提にしていた。しかも #1385 は「#1379 がマージされるまで実装しない」と書いており、古い境界が下流の前提になるところだった
  - 再利用条件: PR をレビューするとき、または ready にする前に、`gh pr view <n> --json commits` の最終 commit の時刻と、関連 issue のコメント・本文の更新（`gh issue view <n> --json comments,updatedAt`）、関連 issue が参照する新しい issue の作成時刻を比べる。PR より後の決定があれば、PR を決定に合わせるのか、決定によって PR が不要になったのか（close が正解）を分けて判断する。並行して設計を詰めているときほど起きやすい
