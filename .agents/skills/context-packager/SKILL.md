...
## Fresh-context handoff rule (#1410)

Allowed Context は **conversation history の圧縮コピーではない**。worker / model / runtime の
変更や独立 review では、保存済み brief/report/evidence と canonical artifact だけを新しい
context に渡す。

- raw chat transcript / hidden chain-of-thought / raw tool stream を package に含めない
- material な decision は canonical Plan / decision-log / ADR を参照する
- progress は `INDEX.md` / `current-state.md` または `dispatch/progress-ledger.md` を参照する
- reviewer には実装会話ではなく review package + diff + evidence を渡す
- missing / stale な canonical state を推測で補って handoff 成功扱いしない

fresh context 側は L0（INDEX/current-state）または task brief を入口にし、必要な evidence を
on demand で取得する。上流 integration map: `docs/ai/context-lifecycle.md`。
