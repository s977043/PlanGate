...
ai-loop を使う場合は、PR 作成後の CI / review repair を `MERGE_READY` まで収束させる。ただし、C-4 / merge は Human-owned のままであり、AI が最終判断を奪うものではない。

## 一言で言うと

PlanGate は、AI が速くコードを書く時代に、PM / PO が守るべき **「何を作るか」「なぜ作るか」「Done とは何か」** を失わないためのゲート型ハーネスである。

AI は実装前に計画と受入条件を出し、標準フローでは人間が承認してからコードを書く。実装後は検証と handoff を残す。
...
AI はまず PBI を読み、計画、TODO、テストケースを作る。標準フローでは、人間が C-3 で承認するまでコードを書けない。実装後は検証、レビュー、handoff まで残す。

ai-loop を使う場合は、PR 作成後の CI / review repair を `MERGE_READY` まで収束させる。ここでも AI の終点は `MERGE_READY` であり、`MERGED` ではない。採用判断と merge は人間が担う。
...
| PR 後の収束を追跡できる | CI / review repair が `MERGE_READY` まで進んだかを判断材料にできる |
...
PlanGate では、AI が実装する前に plan と test-cases を作り、承認してから進めるため、PO は「この PBI は何を満たせば完了か」をコントロールできる。

Phase 0 ultra-light は低リスク・学習目的の例外であり、チーム運用では Phase 1 以降で C-3 / C-4 と検証を段階的に強める。
...
| Phase 0 と矛盾しないか | ultra-light は低リスク・学習目的の例外として説明できているか |
| ai-loop と矛盾しないか | C-3' / `MERGE_READY` / Human-owned merge の境界が明確か |
