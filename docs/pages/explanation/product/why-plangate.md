...
ツールは導入した瞬間が完成形になりがちで、半年後には現場とずれていきます。PlanGate は固定されたワークフローではなく、**製品自身を測りながら改善し続けるハーネス**です。だから、今効くだけでなく、使い続けるほど現場に合っていきます。
...
- **変更を評価で検証する**: 改善仮説を立てて変更し、良くなったかを評価と実利用シグナルで測る
- **効いたものだけ残す**: 良ければ採用、悪ければ巻き戻す
- **学びを次へ回す**: AI の成果物が本当にチームに残ったか（Keep Rate）まで指標として追う

これまでに計画された改善フェーズは、設計・ガバナンス・品質チェックまで含めて**全工程を完走済み**です。「将来の約束」ではなく、すでに走り切った実績がこの改善文化を裏づけます。
...
> 改善ループとロードマップ、およびこれまでの実績の詳細は [Harness Improvement Roadmap][roadmap] を参照してください。
...
| [Harness Improvement Roadmap][roadmap] | 継続改善ループとロードマップ、および実績の詳細 |

[staged-adoption]: ../../../staged-adoption-guide.md
[roadmap]: ../../../ai/harness-improvement-roadmap.md
