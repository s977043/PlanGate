...
| rev8 | **`_PG_CR` / `_PG_TAB` のガードを `\|\|` 前置から `if` ブロックへ**（MAJOR-1 / 挙動同値）/ **サニタイズを共通 helper `_pg_oneline` へ集約し `_esc_dl` / `_esc_c3` の縮退先にも通す**（MINOR-1）/ **§2.4 の検出器の FP・FN 特性を明記** | **§2.4 の「rev7 採用版 hits=0」**（**MN-2 で足した `\|\|` 前置が除外パターンを壊し実測 hits=2。§6 step 4 どおりに実行すると正しい patch の適用が止まる**）/ **`hits=0` を安全の証明として読ませていた書き方**（**行内整形依存のヒューリスティックで FN が実在する**） |
| **rev9**（最終） | **§3(B) hunk 内の shell コメントから展開記法を除去**（MAJOR-1 と同型 / 適用される行に残っていた）/ **§2.4 に「何を対象に測ったか」と「コメントにも記法を書かない」を明記**/ **§6 step 4 の測定対象を明文化** | **rev8 の「hits=0 を実測」**（**測定対象が「私の applier が生成したファイル」で、設計書の hunk そのものではなかった。設計書の `+` 行を全抽出して測ると hits=1**） |
...
#### 測定対象を明示する（**rev8 の食い違いの原因**）

**「hits=0」と言うときは、何のファイルに対して測ったかを必ず添えてください。** rev8 は **AI 側の適用スクリプトが生成したファイル**に対して測って hits=0 と書きましたが、**Human が実際に適用するのは本書の hunk** です。両者はコメント文言が食い違っており、**本書の hunk を測ると hits=1** でした（§3(B) の `(i-d)` コメント）。

**測定対象は次の 3 つを区別すること**:

| # | 対象 | 測り方 | rev9 の実測 |
|---|---|---|---:|
| M-1 | **本書の hunk が追加する行**（適用前に測れる） | 設計書から `^+` 行を全抽出して grep にかける（139 行） | **0** |
| M-2 | **base ∪ 追加行**（適用後ファイルの上位集合） | `cat scripts/hooks/check-plan-hash.sh <抽出行> \| grep …`（519 行） | **0** |
| M-3 | **適用後の実ファイル**（**これが §6 step 4 の対象**） | 適用後の `scripts/hooks/check-plan-hash.sh` に直接かける | 適用時に実行 |

**検出器は行単位のフィルタ**なので `hits(適用後) ⊆ hits(base) ∪ hits(追加行)` が成り立ちます。**M-1 = M-2 = 0 なら M-3 = 0 が保証**されます（削除行は hits を減らす方向にしか働かない）。**rev8 が測っていたのはこのどれでもない第 4 のファイル**でした。
...
| base（現 main） | **0** |
| **rev7**（`\|\|` 前置形） | **2** ← ❌ |
| **rev8 の hunk**（`(i-d)` コメントに展開記法が残存） | **1** ← ❌ |
| **rev9 の hunk（M-1 / 139 行）** | **0** |
| **base ∪ rev9 の追加行（M-2 / 519 行）** | **0** |
| rev9 の切り詰めを無ガードに戻した変異 | **1（kill）** |
...
> **是正は検出器を緩める側ではなく書き方を合わせる側**（`if` ブロック化 / 挙動は同値）を採りました。
>
> **rev9 追記**: 同じ問題が **§3(B) の `(i-d)` コメント**にも残っていました（rev8 では `_pg_oneline` 側のコメントだけ直していた）。**検出器はコメントを除外しない**ため、**コメントも「適用される行」である以上、規則の対象**です。
...
1. **接尾除去 / 接頭除去は必ず `case` で前置ガードする**（本体の規則）。
2. **ガードは `[ … ] ||` 前置ではなく `if` ブロックで書く**（`||` が検出器の除外パターンを壊すため / MAJOR-1）。
3. **コメント内に接尾・接頭除去の記法を書かない。**「接尾除去」「接頭除去」と日本語で書く。**検出器はコメントを除外しないので、コメントも適用対象行として hits に数えられます**（MAJOR-1 / rev9 の再発）。
...
4. **§2.4 の構造検査を `ta-65` に 1 本追加し、ここでも実行する**（未ガードのパラメータ展開の検出 / **wall-clock より決定論的**）。

   **対象は「適用後の `scripts/hooks/check-plan-hash.sh` そのもの」**（§2.4 の M-3）。**設計書でも applier の出力でもありません** — rev8 はここを取り違えて hits=0 と報告し、実際の hunk には 1 件残っていました。

   ```sh
   # ★ step 1 の適用を終えた後、リポジトリの実ファイルに対して実行する
   grep -nE '\$\{[A-Za-z_][A-Za-z_0-9]*(%%?|##?)' scripts/hooks/check-plan-hash.sh \
     | grep -vE '^[0-9]+:[[:space:]]*[^|()]*\)[[:space:]]' \
     | grep -vE 'dirname|:-|:='
   ```

   **hits が 0 でなければ適用を巻き戻す**（検出器を緩めたり例外扱いにしないこと。緩めた時点でこの検出器は 3 回目の O(n²) を止められなくなります）。
   **適用前に確認したい場合**は、本書の hunk から `^+` 行を全抽出して同じ grep にかけてください（§2.4 の M-1 / rev9 実測 **0**）。
