...
### 0-0-8. §1-5「L2 は通常セッションでは 1 件も触れない」の是正（2026-08-26 追記）
...
> **本節の執筆者はこのテスト結果を再実行していない。** 「952 passed / 0 failed」「変異注入で
> 検出力を実証」は **PR #1229 の実行記録が出典**であり、本節の測定基点（`a1b3927`）での
> 実測値ではない。
...
**この構造は本節を追記する作業中にも再現した。** 本節の執筆セッションでは、作業開始直後に
ハーネス側から「ファイル変更は `sed` / heredoc など Bash で行う」旨の運用指示が実際に注入され、
`Write` / `Edit` を使わない経路へ誘導された（依頼側の明示指示を優先して従わなかった）。
**#1104 は仮説上の穴ではなく、通常の作業手順の中で繰り返し発生する経路である。**
詳細は #1104 のコメントを参照。
