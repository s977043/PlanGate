...
| 6 | （補助）③ は「正本修正 + sync で自動追従（手編集しない）」 | sync 経路の実測: `.codex/skills/` = `scripts/install-plangate-skills-to-codex.sh`、`plugin/plangate/**` = `scripts/sync-plugin-plangate.sh` が存在。一方 **`.claude/skills/` を再生成する sync スクリプトは `scripts/` に見当たらない**（**本カウントのスコープは `scripts/` 配下に限定**する〔`grep -rl '\.claude/skills' scripts/` → **5 ファイル**〕。sync スクリプトはすべて `scripts/` 配下にあるため、再生成経路の有無はこの範囲で判定できる。内訳は**書き換え系が rename 用 2 本のみ**〔`apply-diff-audit-rename.sh` / `apply-subagent-team-design-rename.sh`〕で、残りは読み取り専用 checker 2 本〔`check-skill-name-collisions.py` / `check-stale-skill-refs.py`〕とメッセージ文言 1 件〔`scripts/ai-dev-workflow` L96〕が字句参照するのみで再生成はしない。⚠️ **リポジトリ全体では `.claude/skills` の言及は 201 ファイル**〔`docs/working/` と `.claude/worktrees/` を除く `*.md` / `*.sh` / `*.py`〕あり、「5 ファイル全数」はリポジトリ全体の数ではない。TASK-0954 U-3 でも「正本と非同一の別系統」と実測） | ③ 内 `.claude/skills/acceptance-review/SKILL.md` の「sync 経由・手編集ゼロ」（AC-4）の**経路が未確立の可能性** → U-2 として plan で確定 | 要計画（ギャップ検出） |
...
| **② 正本（**原則** AI 編集可・**生成物が 1 件混在**）** | 16 → **16** | `.agents/skills/{plan-review-gate,acceptance-review,README}` / `docs/ai-driven-development.md` / `docs/plangate.md` / `docs/workflows/README.md` / `docs/pages/reference/glossary.md` 等。**⚠️ 層の見出しは「原則」であり、この 16 件には手編集不可の生成物が 1 件含まれる**: ② に含まれる `docs/changelog.md` は `scripts/sync-release-docs.sh` による**自動同期ファイル**（ヘッダに「手動編集しない」明記）かつ「17項目」hit は**過去リリース記録行（L430 付近）のみ** → 字義どおりの ② 統一は生成ファイル手編集 or 履歴書き換えになるため、**④ 相当の不変更 or CHANGELOG 経由 sync のどちらで扱うかを plan で確定**（U-6 に含める） |
