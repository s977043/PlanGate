...
|-------|----------|--------|--------------|-------|
| R-001 | critical | **FIXED** | 本 PR（`_redirect_writes_token` の切り詰めクラス / `T1110-TC-06`〜`TC-08` / 変異 `M-3` `M-4`） | 採用。**推奨案 1（引用・退避が残る先は解決不能として block）を採用**。加えて終端文字クラスから `#` を除去（語中 `#` は退避不要で書けるため、推奨案のままでは残穴だった）。23 ケース + レビューア harness で rc=2 復帰を実測（`evidence/matrix-old-new-fixed.log` / `evidence/v3b-rerun-fixed.log`） |
| R-002 | major | **FIXED** | `evidence/README.md` §R-002 / 変異 `M-3`〜`M-6` | 採用。全称主張を「測定範囲では」のスコープ付きへ書き換え、**レーン内部の分類を壊す変異 4 種**を追加して機械的根拠を付与 |
| R-003 | major | **FIXED**（手続き是正 / 判断は C-3 へ） | `pbi-input.md` §既存 TC の期待値反転 / `plan.md` S-4 🚩 / `test-cases.md` §期待値を変更する既存 TC / `ta-25` の TC コメント | 採用。反転 2 件を明示宣言し、**TASK-1023 AC-04 の redirect レーン限定上書きを Human C-3 の判断事項**として起票。**TASK-1023 側資料の追補は C-3 承認後の follow-up**とし、AI が他 PBI の AC を単独で書き換えない |
| R-004 | minor | **FIXED** | `ta-25` の `T1045-TC-19` コメント / `test-cases.md` | 採用。SC-6 が TASK-1045 exec 中の停止条件であること、#1110 は同じ真の陽性を別経路で維持していること、TC-11〜15 は rc=2 のままであることを明記 |
| R-005 | minor | **FIXED** | `T1110-TC-09`（改行畳み込み）/ `T1110-TC-10`（診断値）/ 変異 `M-5` `M-6` | 採用。V-3 で SURVIVED だった M-C / M-D 相当の変異を追加し、**両方 KILLED** になることを実測 |
| R-006 | minor | **PARTIAL / Human 待ち** | `review-self-2.md`（簡易 C-1 再実行）/ `status.md` | C-1 は再実行して記録。**C-3 は人間必須のため未取得（`c3.json` 未発行）**。`handoff.md` は **C-3 承認後の V-1 完了時に発行**する（現時点で発行すると未承認のまま完了資産を出すことになる） |
| R-007 | info | **ACK（対応せず）** | — | 常時経路は無劣化・超線形でないことを確認済み。hook timeout 設定は `.claude/settings*.json`（Human-owned / 本 PBI Out of scope）。別 PBI 相当という指摘に同意 |
| R-008 | info | **ACK（対象外）** | `evidence/README.md` §本 PBI で閉じていない既知の穴 | 新旧同値の既存穴（外側ゲート `_is_token_path "$_cmd"` 起因）。#1115 として起票済み。**FIXED でも同値であること**を実測し記録 |
| R-009 | info | **ACK** | — | ta-25 は契約導入前のレガシー extras で ta-61 covered set 外。本 PR も 1〜110 行目相当のヘッダ領域に触れていない（fixture / TC の追加のみ）。フルスイートはオーガナイザーが最後に 1 本実行 |

### R-001 で採用しなかった代替案

レビューア推奨案 2（「切り詰めで実際に文字が落ちたかを前後比較し、落ちたレコードは
解決不能扱い」）は**採用しなかった**。レコードは次の `>` またはコマンド末尾まで伸びるため、
`echo x > /tmp/log.txt && git commit -m 'docs: TOKEN'` のような**正当な rc=0 ケースでも
必ず「文字が落ちた」と判定され**、#1110 が解消した誤検出が丸ごと復活する。
採用した案 1 は判定を**切り詰め後の語**に限定するため、この副作用が出ない
（`T1110-TC-01` が回帰ガードとして機能する）。
