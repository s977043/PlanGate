...
- [ ] **T-01** baseline を現 main で再測定する（`sh tests/run-tests.sh` の結果・`ta-65` / `ta-12` / `ta-39` / `ta-45` の PASS 状況）
  - depends_on: なし
  - 🚩 **絶対件数を契約値にしない**。測定環境（OS / シェル / 日時 / main SHA）とセットで記録する
  - `rollback:` 不要（読取のみ）
- [ ] **T-02** 迂回面を現 main で再実測し、`evidence/c2-review/ho-bypass-surface.md` を更新する
  - depends_on: T-01
...
- [ ] **T-03** 正規化関数 `_pg_fold_path()` を**単体ファイル**として実装する（Step 1）
  - depends_on: T-02
...
- [ ] **T-04** `check-plan-hash.sh` への patch を作成する（Step 2）
  - depends_on: T-03
...
- [ ] **T-05** `scripts/apply-1101-ho-normalization.sh` を作成する（Step 2）
  - depends_on: T-04
  ...
  - `rollback:` スクリプトを削除する
- [ ] **T-06** 旧 apply スクリプトの stale 化に対処する（Step 2 / S-2）
  - depends_on: T-04
...
- [ ] **T-07** sandbox 検証環境を作る（Step 3 / R-008）
  - depends_on: T-05
...
- [ ] **T-09** TC-06 を拡充する（Step 4 / AC-3）
  ...
  - `rollback:` 同上
- [ ] **T-10** 直積検証の新 TC を追加する（Step 4 / AC-1）
  - depends_on: T-07
  - 内容: **9 カテゴリ 15 パターン × 変換 7 種 + 2 種複合 → 全件 rc=2**
  - 🚩 **既知 4 ケースの狙い撃ちでは PASS しない**構成にする
  - `rollback:` 同上
- [ ] **T-11** `_norm_target` 不変の回帰表明 3 本を追加する（Step 4 / AC-2）
  - depends_on: T-07
  ...
  - `rollback:` 同上
- [ ] **T-12** fail-closed の新 TC を追加する（Step 4 / AC-8）
  - depends_on: T-07
...
- [ ] **T-18** `docs/ai/hook-enforcement.md` を更新する（Step 9 / AC-7 / S-3）
  - depends_on: T-17
...
- [ ] **H-01** **C-3 ゲート**（**plan v4** の承認 / `c3.json` 発行）
