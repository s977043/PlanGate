...
- **AC-3 の「記録層または契約層」**: W チェック時点で選択肢が残っていたのは事実であり、
  Model B が指摘 → Model D 裁定で**契約層に確定**した（摩擦 F-8 の学習源そのもの）。
  本文を遡及修正すると「B が何を見て reject したか」の監査証跡が壊れるため改変しない。
  学習（配置は W チェック前に 1 つへ確定）は F-8 として次 Run のテンプレートに反映する。
- **AC-2/3 の grep 検証がファイル全体を数える件**: 指摘のとおり §1.1 の既存参照も
  カウントされ「§7 内への追記」の厳密検証にはならない。実際の exec 検証では統合担当が
  §7 表の実体を目視突合して補完した。**F-7（AC 機械検証の厳密化 — 例: §7 範囲を
  sed で切り出してから grep）の具体例**として次 Run の deterministic 検証設計に反映する。
