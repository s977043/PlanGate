...
## 並行セッション発行分の台帳転記（2026-07-08・Run-019）

意味の定義は各出典 run 記録が一次記録・本表は ID 台帳としての索引。並行セッションで
発行された F-35〜F-40 が run 記録のみに存在し台帳に行として欠落していたため、
出典つきで追記する。

| # | 要約（採番元原文に忠実） | 採番元（一次記録） | 正本化 |
|---|---|---|---|
