...
- [x] A-1: baseline 再実測 — clean env で `sh tests/run-tests.sh` と
  ...
  - rollback: 不要（読取のみ）
- [x] A-2: **C-1 セルフレビュー** → `review-self.md`（`C1-VERDICT: PASS plan=sha256:<hash>` マーカー付き）
  - depends_on: A-1
  - 🚩 checkpoint: FAIL があれば plan へ戻す
  - rollback: `review-self.md` を削除して再生成
- [x] A-3: **C-2 外部レビュー 2 レーン**（設計妥当性 / コードベース整合）→ `review-external.md`
...
- [x] A-4: **C-3' 裁定**（Plan Package 検証 → W チェック 2 体 → arbiter 裁定 → record 保存）
  - depends_on: A-2, A-3
...
- [x] A-5: 集計と削除の非対称を解消 — dst 側 stale 集計ループの
...
- [x] A-6: TC-35 追加（symlink stale 混入で「集計 = 削除」を検証。
...
- [x] A-9: AC-1〜4 の突合（V-1 受け入れ検査）→ `status.md` / `current-state.md` 更新
  - depends_on: A-8
  - rollback: 不要（記録のみ）
