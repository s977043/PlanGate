...
|-------|--------|--------------|-------|
| R-001 | open (major) | — | 既定 root 片方不在で検査範囲が半減しても全経路が緑。M-A で実証 |
| R-002 | open (major) | — | TC-10 の `ignored=1` が絶対件数。無関係 PR の Test CI を落とす |
| R-003 | open (major) | — | 既定経路（explicit=False）の検出力が無テスト。M-B で実証 |
| R-004 | open (minor) | — | 2 root の basename 衝突で violation 行が root を区別できない |
| R-005 | open (minor) | — | icon 検査が部分文字列のみ。配布物 root に assets 不在（follow-up 可） |
| R-006 | open (info) | — | 呼び出し元は本 PR 後 3 経路。plan F-7 の doc 鮮度 |
| R-007 | open (info) | — | patch 適用後も sync job は PR で走らない（post-merge only） |
