...
- **実害として顕在化（因果は 3 段階。1 つの出来事に圧縮しないこと）**:
  ...
  2. **是正済み**: 上記の破れは **[PR #988](https://github.com/s977043/PlanGate/pull/988)（`7680145`）が解消**した（ta-58 を `PG_HARNESS_SOURCED` AND 判別へ変更 + standalone 分岐に 7 env unset を追加 / ta-59 は判別式が既に AND で、`unset` の行継続を 1 行化）。`be53897` 時点で **ta-58・ta-59 はいずれも AC-9（＝TC-33 が静的検査する「AND 判別」「7 env unset」の 2 点）を充足済み**である。ただし**規約 8 全体の準拠ではない** — ta-58 には standalone fallback が無く、これが `7dad6dd` の追加分になる（下記）
...
- **info（段階 3 の真因・本 PBI では未対処）**: `ta-60` 単体の不注意ではなく、**`tests/extras/README.md` 規約 8 の例示コード自体が行継続 `\` を使っている**のに TC-33 の旧パーサがそれを読めなかった＝**規約の例と検査器の不整合**である。パーサ側は `7dad6dd` で是正したが、規約の例示と検査器の対応関係を機械的に担保する仕組みは無く、規約が新しい記法を例示すると同種の false positive が再発しうる
