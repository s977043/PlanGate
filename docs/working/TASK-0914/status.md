...
| 2026-08-05 07:05 | **鮮度根拠の是正 + CI 2 failed の解消（PR #986）** | **F-4 の「接触ファイル交差 0」を鮮度根拠とする論法を撤回**。AC-9 はリポジトリ全体の不変条件であり、main が無関係な extras を 1 本追加するだけで破れる ＝ **全体量化子を含む AC の鮮度は接触ファイル交差では担保できない。base 更新のたびに機械ゲートを再実行して判定する必要がある**。実害の因果は **3 段階**（1 つの出来事に圧縮しない）: **(1) 破れ** = main へ ta-58（`c25c022`/#967）と ta-59（`a667c0d`/#976）が入り、ブランチが 1 行も触っていないのに交差 0 のまま AC-9 が破れた（本行の論旨はここで成立）→ **(2) 是正済み** = この破れは **[#988](https://github.com/s977043/PlanGate/pull/988)（`7680145`）が解消**（ta-58 を AND 判別へ + standalone に 7 env unset 追加 / ta-59 は判別式が既に AND で `unset` の行継続を 1 行化）。**`be53897` 時点で ta-58・ta-59 はいずれも AC-9（TC-33 が静的検査する「AND 判別」「7 env unset」の 2 点）を充足済み**（規約 8 全体の準拠ではない — ta-58 の standalone fallback は未了で、それが `7dad6dd` の追加分） → **(3) 別経路での再発** = `ta-60`（#989）が `unset` を行継続で 3 行に分けて持ち込み、TC-33 のパーサが継続行を読めず false positive を出した。**真因は ta-60 単体の不注意ではなく、`tests/extras/README.md` 規約 8 の例示コード自体が行継続 `\` を使っているのに検査器がそれを読めなかった＝規約の例と検査器の不整合**（パーサは是正済みだが、両者の対応を機械的に担保する仕組みは無い — handoff §2 に info として記録）。**これが `be53897` 時点の CI 2 failed（TC-33 + TC-13 連鎖）の直接原因**。是正 3 点: ①ta-26 TC-33 のパーサを awk で行継続結合してから走査（旧実装は `grep '^\s*unset '` 直接のため ta-60 の複数行 `unset` の 2 行目以降が不可視 + 末尾 `\` を env 名として収集 = false positive。ta-60 は 7 env すべて unset 済みで修正不要）②**ta-58 に standalone fallback を追加**（`pass`/`fail`/`register_cleanup` + 末尾 drain・サマリ・exit code）。**AND 判別と 7 env unset は #988 で既に入っており、本コミットの追加分は fallback のみ**（#988 の積み残し。fallback 前は FAIL があっても exit 0 で素通り）③`scripts/sync-plugin-plangate.sh` 経路2 コメントを保証範囲どおりに是正（挙動不変）。実測: フルスイート **538〜539 passed / 0 failed・rc=0**（TC-13 / TC-33 含む。3 者独立実測で 0 failed 一致・総数のみ worktree 538 / primary checkout 539 に振れる = #947 の環境依存変動の再現）/ ta-26 standalone **30/0・rc=0** / ta-58 standalone **40/0・rc=0**（変異注入時 39/1・**rc=1**。修正前 HEAD 版は同変異で 1 FAIL でも rc=0 = 検出力の対比実証） |
...
| 2026-08-05 08:05 | **鮮度規律の自己適用（PR #996）** | 独立敵対レビューの条件 6 件を消化。うち MJ-1（基点 `be53897`）/ MJ-2（state stale）/ MI-1（総数の測定環境）/ IN-8-2（MD032）は前コミットで消化済み、本コミットで MI-2（履歴行の時系列順）・MI-3（AC-9 スニペットへの cross-ref）を追加。**IN-8-3 = 本 PBI 自身が定めた鮮度規律の自己適用**: main が `0ebb8fe` → **`484728f`**（#993 マージ）へ前進したため、接触ファイル交差ではなく**機械ゲートを再実行**して判定 — `sh tests/run-tests.sh` = **539 passed / 0 failed・rc=0**、AC-9 静的検査 = 検査対象 **15 ファイル・単独判別残存 0**。前進分は `AGENT_LEARNINGS.md` のみ（doc-only・`tests/extras/` 不変）だが、規律どおり差分推論で済ませず実測した |
| 2026-08-05 08:20 | **PR #996 レビュー disposition（major 1 / minor 2）** | Copilot は quota 超過で**レビュー不成立**のため一次ソース照合で代替（bot 不在の補完）。**major 1 = 是正内容そのものの原因帰属が誤っていた**: 本 PBI は「原因帰属を誤った鮮度推論」を撤回するために書かれたのに、置き換え先の記述が同じ誤りを含んでいた（「ta-58 は真の未移行〔AND 判別・7 env unset・fallback 欠落〕」「#967 / #976 が入って CI 2 failed」）。`be53897` の一次ソースで反証: ta-58 の AND 判別・7 env unset は **#988（`7680145`）で既に完了**、`7dad6dd` の追加分は **standalone fallback のみ**、CI 2 failed の実因は **ta-60（#989）の行継続 `unset`** による TC-33 false positive。**#988 は handoff / status のどちらにも 0 回しか登場しなかった**（作業の半分を担ったマージ済み PR が恒久記録から欠落）→ 因果を 3 段階（破れ → #988 で是正 → #989 で別経路再発）へ書き直し、#988 を明示。**minor 1** = 本ファイルの「最終更新」が stale（本文は 08-05 更新済みなのに `2026-08-04 09:55` のまま・#463 の趣旨）→ 是正。**minor 2** = C-3' 裁定記録の `target_sha` が `ee3ac9a` で PR head と乖離（`43a4f20` 以降の変更が未裁定のまま同梱）。`decision: HUMAN_ESCALATED` のため承認境界は緩まず実害は低いので、**裁定記録は evidence artifact として改変せず**本行に被覆範囲の制約として記録する（本 PBI が説く鮮度問題と同型の状態が裁定記録自体に生じた例） |
