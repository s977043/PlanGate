...
> 基点: main `90c313d` / C-2 指摘 `R-301..R-309` / `R-350..R-354` 反映済み
...
- [ ] **T-01**: AC-6 の baseline 実測（R-301 / U-4）
  - 移行前の 11 本（ta-39/43/44/45/46/47/49/50/51/52/53）について、clean env（`env -u PLANGATE_HOOK_TASK -u PG_HARNESS_SOURCED -u FIXTURES_DIR`）での standalone 実行時の **`[PASS]` 件数**を実測し `status.md` へ表形式で記録（= AC-6 条件③の baseline）
  - 11 本の**失敗表記が `[FAIL]` に統一されている**ことを grep 実測（`NG` / `ERROR` / `not ok` 等の別表記がないか）。非統一なら AC-6 条件①の判定語彙を拡張して test-cases.md を更新
  - **AC-7 の検出力証明（R-302）**: 移行**前**に汚染 env（`PLANGATE_HOOK_TASK=TASK-9999` 等）で 11 本を実行し、**ta-39 で NG が出ること**を `evidence/test-runs/` へ保存（移行後に NG が消えることで検出力を示す）
...
- [ ] **T-02**: `_mass_delete_blocked()` を追加し、既存 `sync_dir` 内 guard（**L103-113**。内側 `if`〜対応する `fi` 2 個まで — R-352）を同関数呼び出しへ置換
  - 🚩 **チェックポイント**: `sh -n` PASS + `tests/extras/ta-26-plugin-sync.sh` の既存 15 TC 全 PASS（`sync_dir` 経路の挙動不変を証明）
...
> **T-05 は粒度確保のため 3 分割**（C-1 自己検出: 12 TC を 1 タスクにすると独立検証できない）。共通の実装規約は 3 タスクすべてに適用する:
>
...
- [ ] **T-06**: **変異注入で新規 TC の検出力を実証**（M-1〜M-7）
  - guard 弱体化方向（M-1〜M-5）**および過剰発火 / override 無効化方向（M-6 / M-7）**の全 7 変異について、対応 TC が **FAIL する**ことを確認（R-305。M-6/M-7 がないと正常系 TC と override TC の検出力が未実証のまま「実証済み」と記録されてしまう）
...
- [ ] **T-09**: AC-6 / AC-7 / AC-9 の機械検証
  ...
  - **AC-7**: V-1-B（**汚染 env を明示注入する別ループ**。V-1-A の `env -u` を流用しないこと — R-302）で AC-6 と同結果
...
- [ ] **T-10**: 別 issue 起票 + handoff への妥協点記録（AC-8）
  ...
  - 根拠として「`PLANGATE_HOOK_TASK` 汚染下の ta-39 が 6 件 FAIL しつつ exit 0 で通った」実測を添付
...
- [ ] **T-11**: 回帰フルテスト
  - `sh tests/run-tests.sh` が **430 + 新規 TC 数 passed / 0 failed**
...
- 変異注入 **M-1〜M-7** で新規 TC（負側・正常系・override）の検出力が実証済み（evidence あり）
