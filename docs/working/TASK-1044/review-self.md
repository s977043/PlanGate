...
> ⚠️ **機械可読マーカーの非対称について（R-034 / 意図的な選択 = 対応しない）**:
> `^C1-VERDICT:` に一致する行は本ファイル冒頭ラウンドの 1 本のみで、
> **hash は Round 1 時点（`586f8a91…`）に固定**されている。以降の再実行は
> `C1-VERDICT-N`（N ≥ 2）と採番しており **`^C1-VERDICT:` に一致しない**。
> したがって `scripts/ai-loop/plan_package.py` の `check_evidence` から見ると
> **C-2 マーカーは最新 plan_hash と一致・C-1 マーカーは stale** という非対称がある。
> **本 PBI は人間 C-3 経路であり ai-loop の機械 gate を通さない**こと、
> 方向が **fail-closed（安全側）**であること、是正案 (a)（末尾に最新
> `C1-VERDICT:` を足し冒頭を `C1-VERDICT-1:` へ改番）は **過去ラウンドの記録行を
> 書き換える**ことになり本 PBI が通してきた「履歴は書き換えず新節で訂正する」規律と
> 衝突することから、**(b) 対応しない**を選択した。
> **最新の C-1 判定は本ファイル末尾の `C1-VERDICT-N`（= 常にファイル末尾の 1 本。
> 特定の N を本注記に書かない — 書くと注記自体が stale 化する / R-037）を正**とする
> （`review-external.md` の frontmatter が「機械可読な判定は `C2-VERDICT` 行を正とする」と
> 書いているのは **C-2 側のみ**の話であり、C-1 側は本注記を参照すること）。
> ai-loop 経路へ載せる後続 PBI では、**採番せず末尾 1 本の `C1-VERDICT:` を
> 更新し続ける運用**を推奨する。
>
> レビュー日: 2026-08-12
...
## 簡易 C-1 再実行 #6（2026-08-12 / PR 前 River Review 2 回目 R-032〜R-035 の 1 回確定反映後）

> 反映元: **PR 作成前 River Review 2 回目**（`review-external.md`「PR 作成前 River Review
> 2 回目」節 / major 1 / minor 3 / info 1）。**前回の major 2 件（R-024 / R-025）は
> 両方とも実質解消**。新規 major 1 件（R-032）は **R-030 の反映が生んだ副作用**。
> 承認トークンは依然未発行 = plan 編集可能期間内の確定反映（EH-3 mismatch なし）。

### 反映内容の整合確認（R-032〜R-035 全 4 件）

| R | severity | 反映 | 判定 |
|---|---|---|---|
| R-032 | major | AC-3 / TC-34 の導出集合を「**marker を含む `ta-*.sh` 全件 − contract TA 自身（`$_T61_SELF_ID`）**」へ限定し、**自己除外が必須である機構**（`ta-61:784-800` の入れ子フルスイート 2 回起動 → 再帰 / `:60-62`・`:312-314` の `timeout 180` FAIL / TASK-0921 実測 282s・余裕 318s の食い潰し / `ta-61:304` の既存自己除外パターン）を TC-34 に明記。**S6 / T-07 の「層 A 12 本」を同じ動的表現へ**揃え、契約と実行手順の食い違いを解消 | PASS |
| R-033 | minor | AC-2c / TC-31 (3) の判定対象を「**`run-tests.sh` の `^unset` 行から実行時導出した env 集合の全名**」へ。**既存の `_T61_GUARDED_ENVS`（`ta-61:286`・TC-15 が導出自体を検査）をそのまま消費**する形にし、**名前・件数・行番号のいずれも契約値にしない**（7 名固定は 8 個目の guarded env を静かに落とす = R-030 が潰した偽陰性と同クラス。行番号アンカーは本 PBI 自身の「stale 化する」結論と逆行） | PASS |
| R-034 | minor | **(b) 対応しない**を選択（理由は後述）+ **`review-self.md` 冒頭に非対称の注記**を追加し、`review-external.md` の frontmatter コメントからも誘導。監査表の status は **`not-applied（意図的 / (b) を選択）`** と明示 | PASS（意図的な非適用） |
| R-035 | minor | frontmatter `verdict` を **schema enum 準拠の `PASS`** へ（`schemas/review-external.schema.json` の enum = `["PASS","WARN","FAIL"]`）。**`C2-VERDICT` 行の `approve` はレビューア語彙なので現状のまま** | PASS |

### R-034 で (b) を選んだ理由

1. 本 PBI は **人間 C-3 経路**であり ai-loop の機械 gate（`check_evidence`）を通さない。
   非対称の方向も **fail-closed（安全側）**で、誤って通る側に倒れない
2. (a) は冒頭ラウンドの `C1-VERDICT:` 行を `C1-VERDICT-1:` へ**改番する = 過去ラウンドの
   記録行を書き換える**ことになり、本 PBI が C-1 #4 / #5 の「過去記録の訂正」節で一貫して
   守ってきた **「履歴は書き換えず新節で訂正する」規律**と衝突する
3. 代わりに**注記で非対称を可視化**し、「最新の C-1 判定は末尾の `C1-VERDICT-N` を正とする」
   「**ai-loop 経路へ載せる後続 PBI では採番せず末尾 1 本の `C1-VERDICT:` を更新し続ける**」
   という運用推奨まで書いた。**C-3 の人間が誤読しない状態**を優先した

### 全数照合の実測（R-032 の教訓 = パターンを「接尾辞なしの短い形」へ）

**前回の失敗**: C-1 #5 は `層 A 12 本の standalone` というパターンを使ったため、
実文字列 `層 A 12 本の清浄 env standalone` に **1 語差で当たらず**残骸が生存した。
**母数（6 ファイル）ではなくパターン設計が弱点**だった。

**今回の手順（2 段）**:

**(1) 短形パターンの全ヒットを列挙して個別分類**（`層 A 12 本` / `18 本` / `4 本` /
`tc26-runner` を live claim 6 ファイルへ）— ヒット総数 **29 行**（plan 10 / test-cases 8 /
pbi-input 4 / todo 3 / current-state 3 / INDEX 1）を**全件目視分類**し、
**残骸 0 件**を確認した:

| 分類 | 内容 | 件数 |
|---|---|---|
| **正当** | `4 本` = 再走バケット（M-01/02/03/16）または「挙動が変わる部分集合（走査母数ではないと明示）」 | 多数 |
| **正当** | `18 本` = **「stale 値である」と否定する文脈**（R-025 の訂正記述）のみ | 全件 |
| **正当** | `tc26-runner` = 「TC-37 の literal フィルタに**マッチしない側**」の構造説明のみ | 全件 |
| **正当** | `層 A 12 本` = **本 PR で touch するファイルの列挙**（pbi-input Context / In scope / plan Files 節）。**AC の判定母数ではない旨を明記**（R-030・R-032 の注記を今回追加） | 3 |
| **残骸** | — | **0** |

**(2) 「否定済み概念が live 主張として残っていないこと」を機械判定**（10 パターン）—
**合計 0 件（exit=1）**:

| パターン | 件数 |
|---|---|
| `層 A 12 本の清浄` | 0 |
| `層 A 12 本の standalone` | 0 |
| `fixture 4 本` | 0 |
| `4 本の完全列挙` | 0 |
| `変異 evidence 18 本` | 0 |
| `18 本のうち 14` | 0 |
...
> **規律として確立**: 量化子を書く前に **(1) 短形で全ヒットを出して個別分類 →
> (2) 否定済み概念の 0 件を機械判定** の 2 段を踏む。**(2) だけでは接尾辞違いで
> すり抜ける**（R-032 の実例）。
...
| 区分 | 判定 | 根拠 |
|---|---|---|
| C1-PLAN-01（受入基準網羅性） | PASS | AC↔TC orphan 0 件を維持。AC-2c / AC-3 の判定条件変更は TC-31 (3) / TC-34 へ双方向反映済み |
| C1-PLAN-02（Unknowns 処理） | PASS | 裁定事項は 5 件で不変。**R-032 は Q 化せず AC 本文で決着**（River Review も「集合帰属は AC-3 の自己除外明記で決着。C-3 に上げる必要なし」と判定） |
| C1-PLAN-03（スコープ制御） | PASS | scope 不変。自己除外は**実装可能性の必須条件**であり scope 拡大ではない |
| C1-PLAN-04（テスト戦略） | PASS | **TC-34 が実装可能になった**（自己除外なしでは再帰 or 180s timeout FAIL で原理的に実行不能だった）。AC-2c は既存 `_T61_GUARDED_ENVS` を再利用でき**新規実装をむしろ減らす** |
| C1-PLAN-05（WBS Output） | PASS | S6 の実行対象を動的表現へ更新。他 Step の Output は不変 |
| C1-PLAN-06（依存関係） | PASS | 不変 |
| C1-PLAN-07（動作検証自動化） | PASS | TC-34 / TC-31 (3) とも動的導出で自動。TC-38 のみ手動（V-1） |
| C1-TODO-01〜05 | PASS | タスク数 12 で不変（T-07 の実行対象記述のみ更新） |
| C1-TC-01〜03 | PASS | TC-31 (3) / TC-34 を AC と双方向更新 |
| C1-SUP-01（Mode 妥当性） | **WARN** | 前回と同じ（Q-3 (1)(2) へ委譲）。本反映も docs のみで `tests/` の touch 対象は不変（15 ファイル） |
| C1-SUP-02（正本・既存ルール整合） | PASS | **repo の確立パターン（`ta-61:304` の自己除外 / `:286` の実行時導出）に AC を寄せた**ことで、契約と実装の乖離が 2 箇所減った。追記専用規約を遵守（本ラウンドは純追記 + frontmatter 値 + `C2-VERDICT` の hash 更新のみ） |

### 判定

- **PASS（WARN 1 / C1-SUP-01）**。WARN は Q-3 として C-3 裁定へ委譲済みでブロッカーではない
- **R-034 は意図的な非適用**（監査表 status に明示 / 理由は上記 + review-self 冒頭注記）
- **info（`TASK-0921/handoff.md` 既知課題 2-bis 本文の 4 つ目の stale「18」）は対応任意**と
  判定されたが、**T-11 の追記先が同じ段落**であり 19 本の注記が並ぶため実害小。
  T-11 実施時に同段落内で自然に是正されうる（AC 化はしない）
- C-3 で裁定すべき項目: **Q-1 (1) / Q-1 (2) / Q-3 (1) / Q-3 (2) / Q-4** の 5 件（不変）
- **承認トークンは本反映後の plan_hash（下記）で発行すること**

C1-VERDICT-7: PASS plan=sha256:53ed25957c9c89ad02dbfb715cf90cdf53d66d14492b9ebd8e060b7b69d7bd5e

---

## 簡易 C-1 再実行 #7（2026-08-12 / PR 前 River Review 3 回目 R-036〜R-038 の 1 回確定反映後）

> 反映元: **PR 作成前 River Review 3 回目**（`review-external.md`「PR 作成前 River Review
> 3 回目」節 / major 1（PR ブロッカー）/ minor 2）。**「是正が新たな major を生む連鎖は
> 止まっている。新規 major は今回の反映の副作用ではなく R-029（1 回目）の取りこぼし」**と判定。
> 承認トークンは依然未発行 = plan 編集可能期間内の確定反映（EH-3 mismatch なし）。

### 反映内容の整合確認（R-036〜R-038 全 3 件）

| R | severity | 反映 | 判定 |
|---|---|---|---|
| R-036 | major（PR ブロッカー） | `plan.md` Testing Strategy の AC-2c 節を書き換え — 見出しを「**guarded env unset の実測**」へ、本文を「**`^unset` 行から実行時導出した `_T61_GUARDED_ENVS` の全名が子で未設定**であることを TC-31 (3) で検証。**`^PLANGATE_` の全数 0 は判定に使わない**（R-029・R-033）。名前も件数も行番号も契約値にしない」へ。**plan 内の AC-2c 言及はこの 1 行だけ**であり、R-029 / R-033 が plan へ一度も反映されていなかった状態を解消 | PASS |
| R-037 | minor | `review-self.md` 冒頭の R-034 注記から**特定の N を除去** — 「`C1-VERDICT-2` 〜 `C1-VERDICT-6`」→「`C1-VERDICT-N`（N ≥ 2）」、「最新は `C1-VERDICT-6`」→「**最新は常にファイル末尾の `C1-VERDICT-N`。特定の N を本注記に書かない（書くと注記自体が stale 化する / R-037）**」 | PASS |
| R-038 | minor | TC-31 (3) に **`_T61_GUARDED_ENVS` の `head -1` 制約**（先頭 1 行しか読まない / sed が行末 `2>/dev/null` を前提）と「**実装時に `head -1` を外すか `^unset` 行が 1 行であることを併せて assert する**」を追記。**TC-15 の表明を「導出が空でないこと（`[ -n … ]`）のみ検査。集合の完全性は未検査 = 従来の『導出自体を検査済み』は過大表明」へ弱めた**。同族残差（マッピング行の `AC-2c（7 env unset の実測）` / 本文の `7 env 契約の外` / TC-31 前文の `7 env unset` / pbi-input 実測 2 の `7 env unset 不発`）も掃除 | PASS |

### 全数照合の実測（**3 段**へ拡張 / R-036 の教訓）

**前回までの弱点**: 2 段手順（短形 → 否定済み概念の機械判定）は、
**短形パターン集合が手選びで「過去ラウンドで否定した概念」からしか作られていなかった**ため、
**当該ラウンドで非契約化した語**（`7 env` / `7 個` / `全数 0` / `grep -c '^PLANGATE_`）が
候補に入らず、**`plan.md` の Testing Strategy とマッピング行がすり抜けた**（R-036 の実測）。

#### 段 1: 過去ラウンドで否定した概念（短形 10 パターン）

live claim 6 ファイルへ `grep -rn` → **0 件（exit=1）**
（`層 A 12 本の清浄` / `層 A 12 本の standalone` / `fixture 4 本` / `4 本の完全列挙` /
`変異 evidence 18 本` / `18 本のうち 14` / `TC-01b/01c が kill` /
`3 条件すべてに検出力があること` / `裁定 4 件` / `実測母数 14`）。

#### 段 2: **本ラウンドで否定・非契約化した語**（新設 / R-036 の直接原因）

`7 env unset の実測` / `7 個の名前` / `全数 0 であること` / `全数 0 を要求` /
`AC-2c（7 env` / `7 env 契約の外` / `導出自体を検査済み` を live claim 6 ファイルへ →
**ヒット 1 行のみ**で、それは **R-038 の訂正文そのもの**
（「…集合の完全性は検査していない — 『導出自体を検査済み』は過大表明だった」）＝**残骸 0 件**。

#### 段 3: **AC ID 軸の横断照合**（新設 / 「反映したファイル」ではなく「AC ごとに全ファイル」）

本ラウンド群で変更した **AC-2c / AC-3 / AC-4 / AC-8 / AC-9** について
`grep -n 'AC-XX' plan.md pbi-input.md test-cases.md todo.md` の**全ヒットを読んだ**:

| AC | plan | pbi-input | test-cases | todo | 判定 |
|---|---|---|---|---|---|
| **AC-2c** | 1 | 1 | 2 | 0 | **是正済み**。plan の 1 行が R-036 の対象そのもので、今回 guarded env 実行時導出へ書き換え。test-cases はマッピング行 + TC-31 (3) 本文で一致 |
| **AC-3** | 3 | 2 | 3 | 1 | 一致。自己除外（`$_T61_SELF_ID`）が pbi-input AC-3 / TC-34 / S6 / T-07 の 4 箇所すべてに載っている |
| **AC-4** | 9 | 6 | 3 | 1 | 一致。**「リスト」表記が残っていた 2 箇所（正本管理表・S5）を「marker の各出現（`file:line`）／ファイル単位リストにしない」へ是正**（本ラウンドで自ら検出） |
| **AC-8** | 13 | 5 | 6 | 2 | 一致。走査母数は全箇所で「`. "$T61_HELPER"` 由来の動的導出・実測 12 本は非契約」 |
| **AC-9** | 7 | 2 | 3 | 2 | 一致。分母 19 / 15 superseded + 4 再走 / 意味ラベル anchor が全箇所で揃っている |

> **段 3 の収穫**: R-036 の対象（plan の AC-2c）に加え、**AC-4 の「リスト」表記 2 箇所**を
> 自力で検出できた。**段 1・2 では当たらないクラス**（否定語ではなく**単位の言い換え**）であり、
> **AC ID 軸でなければ捕まらなかった**。3 段目の有効性が同一ラウンド内で実証された。

### 17 項目の再判定

| 区分 | 判定 | 根拠 |
|---|---|---|
| C1-PLAN-01（受入基準網羅性） | PASS | AC↔TC orphan 0 件を維持。**AC ID 軸の横断照合で AC-2c / AC-3 / AC-4 / AC-8 / AC-9 の全ヒットを読み、契約文と Testing Strategy / WBS / todo の齟齬が 0 であることを確認** |
| C1-PLAN-02（Unknowns 処理） | PASS | 裁定事項は 5 件で不変（Q-1 (1)(2) / Q-3 (1)(2) / Q-4） |
| C1-PLAN-03（スコープ制御） | PASS | scope 不変。本ラウンドは文言是正のみで AC / TC の追加なし |
| C1-PLAN-04（テスト戦略） | PASS | **plan の Testing Strategy と AC / TC が初めて完全整合**（R-036 まで plan だけ旧判定式だった）。TC-31 (3) には `head -1` 制約という**実装時の落とし穴**まで書かれた |
| C1-PLAN-05（WBS Output） | PASS | 不変 |
| C1-PLAN-06（依存関係） | PASS | 不変 |
| C1-PLAN-07（動作検証自動化） | PASS | 不変 |
| C1-TODO-01〜05 | PASS | todo は本ラウンド変更なし（タスク 12 本） |
| C1-TC-01〜03 | PASS | TC-31 (3) / マッピング行を AC-2c と双方向更新 |
| C1-SUP-01（Mode 妥当性） | **WARN** | 前回と同じ（Q-3 (1)(2) へ委譲）。`tests/` の touch 対象は不変（15 ファイル） |
| C1-SUP-02（正本・既存ルール整合） | PASS | **委譲先の実装制約（`head -1`）を契約文に明記**したことで「既存機構を消費する」という設計の穴が閉じた。TC-15 の過大表明も是正。追記専用規約を遵守（本ラウンドは純追記 + `C2-VERDICT` の hash 更新のみ） |

### 判定

- **PASS（WARN 1 / C1-SUP-01）**。WARN は Q-3 として C-3 裁定へ委譲済みでブロッカーではない
- **PR ブロッカー（R-036）は解消**。River Review は「上記 2 行を直し AC ID 軸の横断照合を
  1 回実行すれば収束と判断してよい」としており、**3 段照合を実行して結果を本節に記録した**
- C-3 で裁定すべき項目: **Q-1 (1) / Q-1 (2) / Q-3 (1) / Q-3 (2) / Q-4** の 5 件（不変）
- **承認トークンは本反映後の plan_hash（下記）で発行すること**

C1-VERDICT-8: PASS plan=sha256:24f3faf99c427637f377e6580dc7f0667d2e103aa2f2fd0ee7adb461183f8bb4
