...
- glob 照合なら bootstrap 14 箇所すべてが同一バイト列のまま（AC-4。helper は
...
- **bootstrap（トップレベル / 14 箇所バイト一致）**: `$0` を case で評価し
...
**DoD（AC-4 の照合規約）**: (1) bootstrap の判定 2 行（case 行 + if 行）が
**14 箇所**（層 A 12 + ta-61 本体 + ta-61 fixture 複製）で**行頭空白を除去した上で
バイト一致**。(2) helper は照合対象から**分離定義**とし、上掲の変数消費形 literal と
一致することを別途照合する。

**帰結（exec で必ず対応）**: helper init を harness mode で通したい fixture
（ta-61 の tc01.sh / tc01b.sh 等、bootstrap を持たず helper を直接 source する形）は
`_pg_extra_direct` 未設定 → 既定 direct=1 → standalone に落ちるため、
**harness を模す fixture は `_pg_extra_direct=0` を明示設定する更新が必要**（S5 / TC-36）。

bootstrap 全体（14 箇所で全体バイト一致）:
...
| 参照の切替 | **TC-35 新設**（base の ta-61 に述語バイト一致 TC は存在しないため「更新」ではなく新設。AC-4 の bootstrap 14 箇所一致 + helper 分離照合）+ `_extra-contract.sh` ヘッダコメントの正本参照を本 plan へ更新 |
...
| S5 | 述語機械照合 TC-35 を**新設**（base の ta-61 に述語バイト一致 TC は存在しない。bootstrap 2 行 × 14 箇所 + helper 変数消費形の分離照合）+ F-3 用 TC-32（init 前 finalize → exit 4 + 診断）+ **harness 模擬 fixture（tc01.sh 等）へ `_pg_extra_direct=0` 明示設定** | ta-61 差分 | agent | 低 | |
...
| S8 | 4 シェルマトリクス最終実測（dash/zsh/bash/sh × 実測 1/2 シナリオ）+ handoff 対応表（**TASK-0921 handoff の「14 箇所」と本 plan の分母差 — 旧 14 = 層 A 12 + ta-61 本体 + helper、新 = bootstrap 14（fixture 複製を含む）+ helper 別枠 — を 1 行注記**） | evidence + handoff 素材 | agent | 低 | |
...
- **機械照合（TC-35 新設）**: bootstrap 判定 2 行の **14 箇所**バイト一致
  （**行頭空白を除去して比較** — fixture 複製はインデント差がありうるため正規化規約を固定）
  + helper 変数消費形の**分離照合**を grep ベースで TC 化（AC-4。絶対件数でなく
  「照合対象リストとの同値」で書き、成長ディレクトリの件数固定を避ける）
