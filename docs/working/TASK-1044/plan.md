...
| 参照の切替 | **TC-35 新設**（base の ta-61 に述語バイト一致 TC は存在しないため「更新」ではなく新設。AC-4 の bootstrap marker **各出現**での一致（ファイル単位リストではない / R-024）+ helper 分離照合）+ `_extra-contract.sh` ヘッダコメントの正本参照を本 plan へ更新 |
...
| S5 | 述語機械照合 TC-35 を**新設**（base の ta-61 に述語バイト一致 TC は存在しない。bootstrap 2 行 × **marker の各出現**（`file:line` 単位の動的導出。ファイル単位リストにしない / R-005・R-024）+ helper 変数消費形の分離照合）+ F-3 用 TC-32（init 前 finalize → exit 4 + 診断）+ **helper を直接 source する全 fixture（本 PR 時点の実測 12 本・`. "$T61_HELPER"` 由来で動的導出）へ `_pg_extra_direct=0` を明示設定**（うち挙動が変わるのは `tc01.sh` / `tc01b.sh` / `tc21.sh` / `tc26`（file1 側）の部分集合。harness 模擬・standalone 期待の双方 / R-001・R-002・R-014）+ **AC-8 静的検査 TC-37 を新設**（`_pg_extra_direct` 未設定 fixture = 0 件）+ TC-30b（`_pg_extra_direct=0` を export しても層 A は standalone / R-008）+ `tests/extras/README.md` 規約 8 へ 1 行追記（トップレベル設定必須 / R-012。Q-2 を「追記する」で確定） | ta-61 差分 + README 差分 | agent | 低 | |
| S6 | green 確認: 新 TC + ta-61 全 TC + フルスイート `sh tests/run-tests.sh` rc=0 + **marker を含む `tests/extras/ta-*.sh` 全件 − contract TA 自身**（動的導出・件数 assert なし。本 PR 時点の実測 12 ファイル / R-030・R-032） の清浄 env standalone 実行（**ta-61 自身は除外 — 入れ子フルスイート再走で再帰 / 180s timeout FAIL になるため** / R-032） | `evidence/test-runs/post-fix-*.log` | agent | 低 | 🚩 |
...
- 層 A 12 本: `ta-39` `ta-40` `ta-43` `ta-44` `ta-45` `ta-46` `ta-47` `ta-49` `ta-50`
  `ta-51` `ta-52` `ta-53`（bootstrap ブロック同型置換のみ。**これは本 PR で touch する
  ファイルの列挙であって AC-3 / AC-4 / AC-8 の判定母数ではない** — 各 AC の母数は
  marker / `. "$T61_HELPER"` から動的導出する / R-030・R-032）
...
- **guarded env unset の実測（AC-2c）**: `tests/run-tests.sh` の `^unset` で始まる行から
  **実行時導出**した guarded env 集合（`_T61_GUARDED_ENVS` / `ta-61:286`）の**全名**が
  standalone 契約下で起動した子プロセスで未設定であることを **TC-31 (3)** で検証する。
  **`^PLANGATE_` の全数 0 は判定に使わない**（repo 内の `PLANGATE_*` は実測 52 種で、
  `PLANGATE_BIN` / `PLANGATE_PYTHON` / `PLANGATE_REPO_ROOT` 等は guarded 集合外。
  全数 0 にすると**無関係な `PLANGATE_*` を export した開発者環境・CI で落ちる** /
  R-029・R-033）。**名前も件数も行番号も契約値にしない**。
  この検証が無いと**漏出 env が子へ伝播したままでも TC-31 が緑**になる（R-004）
