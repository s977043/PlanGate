...
| R-003 (codex#3) | 設計 | major | Jekyll build を Human optional ではなく **mandatory pre-C-3 gate** に格上げ。`cd docs && bundle exec jekyll serve --baseurl /PlanGate` で `/PlanGate/pages/...` (Pages source=/docs ゆえ /docs/ prefix なし) 各 link が 200 OK を確認するまで C-3 ゲート進めない | _本コミット_ | reflected |
...
## Codex 最終確認レビュー (2026-05-26) follow-up

| ID | 内容 | reflected_in |
|----|------|--------------|
| R-011 (codex final) | Testing Strategy / TODO H-02 / review-external R-003 で `/docs/pages/...` と `/PlanGate/pages/...` が混在 → 全て `/PlanGate/pages/...` に統一 (Pages source=/docs ゆえ /docs/ prefix なし) | _本コミット_ |
| R-012 (codex final) | Hard pre-C-3 gate の実行条件未確定 (Gemfile 不在、baseurl 設定なし) → `cd docs && bundle exec jekyll serve --baseurl /PlanGate` 明示、Gemfile 不在時の事前準備手順を Testing Strategy に追記 | _本コミット_ |
