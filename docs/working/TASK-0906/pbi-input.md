...
> Issue: [#906](https://github.com/s977043/plangate/issues/906)（ai-loop / governance）— 「arbiter が導入先 `ho-paths.md` §2（app コード / 共有 Model / 認証等）を自動検知せず、app コード拡張時の §2 ガードが手動運用頼み」
...
| 8 | `escalate_paths` / `deny_paths` は現存しない | `grep -rn "escalate_paths\|deny_paths"` → ヒットは `docs/working/TASK-0916/pbi-input.md` の 2 行のみ。`scripts/ai-loop/arbiter.py` = **0 件** / `docs/workflows/ai-loop/loopspec.md` = **0 件** | **確認**（提案 (B) は完全新規フィールドの追加になる） |
| 9 | テスト baseline | `python3 scripts/ai-loop/test_arbiter.py`（**リポジトリルート起点必須**） | **247 tests OK**（実測） |
...
3. **負側テストの追加**（`test_arbiter.py`）
   ...
   - **`allowed_paths` に §2 パスを宣言していても（＝宣言漏れの逆パターン・`scope_violation` が発火しない状況でも）escalate されること**（issue の中心的な盲点＝宣言漏れ耐性の固定）
...
- **`plugin/plangate/skills/ai-loop-cycle/**` 配布派生の独立監視**（既存 sync drift-check の担当）
...
- **AC-9**: テスト 2 系統がいずれも green
  - `sh tests/run-tests.sh`（PlanGate 本体 harness。**arbiter のテストは含まれない**）
  - `python3 scripts/ai-loop/test_arbiter.py` = **247 tests OK** + 新規追加分
...
**(B) を採るべきケース（plan で棄却理由を残す）**: run ごとに escalate 対象を変えたい要求が出た場合。ただしその要求は現時点で観測されていない。
...
- ただし同 L57 が明記するとおり、これは現状 **規範層**である: 「arbiter（`arbiter.py`）の `boundary_check` は ho-paths.md の HO 表からのみ touches-HO を導出するため、上記 carve-out パスは現状 **boundary=clean と判定される**（機械層では escalate しない）。よって本 carve-out は**規範層**であり、eligible 判定時に**実行者が escalate する責務を負う**」
...
| **R-6** | **導入先が `ho-paths.md` を配置し忘れた場合、arbiter は plugin 同梱の plangate 用雛形（21 パターン）を解決してしまい、`patterns` が非空のため priority 0（fail-closed）が発火しない** | 導入先の §1 も §2 も保護されないまま `clean` 判定で run が進む（実測: 雛形 21 パターンに対し `['app/Models/Service.php', '.env']` は `clean`） | 本 PBI の直接スコープ外だが、§2 接続時に同経路を触るため plan で扱いを判断する（別 issue 起票も可）。現状の防御は `execution-runbook.md` L14-18 の**規範**（「未確定のまま run を開始してはならない」）のみ |
...
- **U-8**: **R-6（雛形フォールバックによる silent clean）を本 PBI で扱うか、別 issue に切り出すか**
...
- テスト baseline: `python3 scripts/ai-loop/test_arbiter.py` = **247 OK**（main `c474b70` で実測。リポジトリルート起点必須）
