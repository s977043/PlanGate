...
- **carve-out は列挙でなく glob（Human 決定 2026-07-24）**: PR 前敵対レビューで列挙方式が判定正本 7 件を取りこぼし自己矛盾（R-108）していたため、`docs/workflows/ai-loop/**` + `docs/ai/ai-loop/**` の glob へ変更。トレードオフとして**従来 dogfooding 域（ai-loop 純ドキュメント）の auto-approve も escalate になる**が、安全性優先で受容（ai-loop 外の本体 code auto-approve = 本 PBI の主目的は不変）。
