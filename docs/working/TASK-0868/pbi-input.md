...
3. **dead reference 解消**: `.agents/skills/` の 3 SKILL（subagent-dispatch / subagent-team-design / context-packager）の `plugin/plangate/rules/subagent-roles.md` 参照を再建後の role 正本へ張り替え、各コピー（`.claude/skills/` / `.codex/skills/` / `plugin/plangate/skills/`）へ**それぞれの同期経路で**反映する（経路は単一 sync ではない。実測: `sync-plugin-plangate.sh` は `.agents/skills` → `plugin/` のみ、`.codex` は `install-plangate-skills-to-codex.sh` の別経路、`.claude/skills` は runtime copy。C-3 論点 6）
4. **dispatch-template への routing decision 参照欄追加**: 要素 2「格上げ理由」を routing contract 参照へ拡張（自由記述→構造化。既存 8 要素は削らない additive）
5. **plangate-flow-integration へのルーティング判断フロー追記**: mode / role / verifier 要否 / escalation のオーケストレータ判断責務（issue 検討方針 2）と、mode 別の既定値継承 vs 明示必須の境界（検討方針 3 verbatim: ultra-light / light は mode 由来の既定値を許容、**standard 以上、write あり、review=true、high-risk / critical は明示指定を必須化**、親モデルの暗黙継承を許可する条件を限定、critical で未対応・unknown profile は fail closed または human escalation）
...
8. **routing 記録フィールド案の定義**（issue 検討方針 6 の v1 範囲）: requested/resolved profile・effort・role・reasons・verifier・escalation・outcome を含むフィールド案を routing-contract に定義（**plan で issue 検討方針 6 の全記録候補 — task_mode / retry count / human intervention / 公開安全なコスト・利用量メタデータ等 — を突合して確定**する）。実記録先は #874 RunEvidence `routing_decisions[]` / #811 Trust Ledger が未実装のため、**AC-6 の充足手段は C-3 論点 4 で確定**する（(a) を採る場合は最小記録 producer を本 In scope へ繰り入れる）
...
- AC-2: high-risk / critical で軽量 profile が誤選択された場合に block または escalation できる（実現地点 — dispatch 時検査 / validator + fixture / 文書判定基準 — は C-3 論点 5 で確定）
...
- AC-6: 実行ログまたは Trust Ledger から routing decision と outcome を追跡できる（v1 での充足手段は C-3 論点 4 で確定。フィールド案定義のみでは本 AC は未達であることに留意）
...
- AC-10（In scope 2/3 対応）: role 正本が再建され、`plugin/plangate/rules/subagent-roles.md` への dead reference が **4 コピーすべての実体**（`.agents/skills/` / `.claude/skills/` / `.codex/skills/` / `plugin/plangate/skills/`）で grep 0 件になり、かつ各同期経路の実行後に 4 実体の該当 SKILL 内容が一致する（drift なし）ことを検証する
...
- **記録先の依存関係**: #874 RunEvidence（`routing_decisions[]` を要求・pbi-input 作成済み・粒度は暫定）と #811 Trust Ledger はいずれも未実装・OPEN。`routing_decisions[]` の**共通 field 名・型・versioning owner は未確定**のため、#874 と共通 field map を確定してから consumer 依存を固定する（#874 側 schema/fixture に #868 必須フィールドを明示。C-3 論点 7）。AC-6 の v1 充足手段は C-3 論点 4
- **#866 との関係**: 本 PBI の対象 skill（subagent-dispatch / subagent-team-design / context-packager）は #866 の対象（intent-classifier / skill-policy-router）と非重複で独立だが、「`.agents/skills` = sync 正本」宣言の是正という同一機構に触れるため、**sync 正本宣言の扱いは #866（OPEN）の確定に追従**する
...
- routing 記録の実記録先・粒度・共通 field map（#874 `routing_decisions[]` / #811 Trust Ledger）→ **C-3 論点 2/4/7**
- effort parity の表現（実効値フィールド vs ランタイム別解決規則）→ plan で確定（R1）

### Assumptions

- 新設: `docs/ai/subagent-delegation/routing-contract.md` + role 正本 1 本。修正: `.agents/skills/` 3 SKILL・`dispatch-template.md`・`plangate-flow-integration.md`・`model-profiles.md` §11（additive）。コピー 3 系統は各同期経路で追従（`plugin/` = `sync-plugin-plangate.sh`、`.codex/skills/` = `install-plangate-skills-to-codex.sh`、`.claude/skills/` = runtime copy。手編集しない。実行順と drift 判定は C-3 論点 6）
...
1. **role 正本の置き場所**: `.agents/skills/` 正本 + 同期（#862 の orphan SKILL 移設と同じ流れ・推奨）か、`docs/ai/subagent-delegation/` の契約文書か（#866 の正本宣言確定に追従）
2. **routing 記録先**: #874 RunEvidence `routing_decisions[]` / #811 Trust Ledger のいずれも未実装。実記録の接続範囲を確定する（論点 4/7 と併せて判断）
3. **v1 非 HO 構成の承認**: `.claude/agents/*.md` / `schemas/**` に触らない構成（HO 導線は apply-script 分離）で AC-1〜10 を満たすことの確認
4. **AC-6 充足手段の選択**: (a) v1 で最小の記録 producer を scope 化する（実行ログ 1 経路・例: `decision-log.jsonl` への `routing_decision` 行追記）/ (b) issue 合意のうえ AC-6 を契約定義に限定し、実記録は #874/#811 の後続 AC へ移管する。(a) 採用時は In scope 8 へ producer を繰り入れ
5. **AC-2 の検証地点**: dispatch 時検査（hook/CLI）か、validator + fixture を v1 scope に含めるか、v1 は文書判定基準のみで機械検証は v2 送りか。**dispatch 時検査等で HO パス（`scripts/hooks/**` / `bin/plangate` 等）に触れる実装を選ぶ場合は Mode 再判定（HO 発火・critical 側への引き上げ）が必要**
6. **4 コピーの正本・同期責務・実行順・drift 判定**: `.agents/skills/`（正本）→ `plugin/`（sync-plugin-plangate.sh）/ `.codex/skills/`（install-plangate-skills-to-codex.sh）/ `.claude/skills/`（runtime copy）の 3 経路が独立しているため、実行順・drift check の担保方法・AC-10 の検証手順（4 実体 grep 0 件 + 同期後一致）を確定
7. **#874 との共通 field map**: `routing_decisions[]` の field 名・型・versioning owner を #874 と合意してから consumer 依存を固定する（#874 の schema/fixture に #868 必須フィールドを明示）
