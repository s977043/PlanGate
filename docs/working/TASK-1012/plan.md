...
> **改訂 9**: C-1 を 9 ラウンド実施し、計 55 件の指摘を反映。さらに **C-2 の指摘を 1 回だけ確定反映**（`Refs: R-001 R-002 R-004 R-005 R-006 R-007 R-008 R-009`。**R-003 は Human C-3 の判断事項として保留**）。ゲート戦略は **C-3'（非 production・裁定記録）+ Human C-3（承認）**。
...
## C-2 指摘の反映（1 回確定・`Refs: R-001 R-002 R-004 R-005 R-006 R-007 R-008 R-009`）

> 集約元は `docs/working/TASK-1012/review-external.md`（**追記専用**。本節は反映側の台帳であり、あちらを編集しない）。
> **R-003（Mode 判定の計数規約）は本反映の対象外** — standard 継続 / high-risk 引き上げの判断は承認境界に関わるため **Human C-3 の判断事項として保留**する。したがって「Mode 判定」節は改訂 8 のまま変更していない。
...
| **T-04** | **変異検証 3 種**（1 つずつ入れて戻す）。①条件反転（**新規 2 ゲート限定**）→ AC-2 が FAIL ②ゲート B 終端を TC-36 手前へ → AC-1 が FAIL ③**ゲート外に、ゲート B 終端付近で定義される変数 `_t26_tgt36` を参照する 1 行を注入** → T-01 の越境検査が ≥1 件（C-2 R-002b） | `evidence/test-runs/t04-mutations.log` | agent | **高** | 🚩 3 変異すべてで期待 FAIL + 各復元後に再 PASS |
...
| **検出力の実証** | **変異 3 系統**（T-04）。①条件反転 → AC-2 が FAIL ②ゲート B 終端の縮小 → AC-1 が FAIL ③**ゲート外から、ゲート B 終端付近で定義される変数 `_t26_tgt36` を参照する 1 行を注入** → 越境検査が ≥1 件。③は当初「ヘルパー定義をゲート内へ移す」としていたが、**同関数の参照はすべてゲート B の内側（562-713）にあるため越境が発生せず空振り**だった（C-1 R3 が実測で検出）。人工的な外部参照の注入に変更し、検査そのものの検出力を直接実証する。**さらに注入対象を `_t26_t20`（ゲート A 先頭付近 L423）から `_t26_tgt36`（ゲート B 終端付近 L721）へ変更した**（C-2 R-002b）: 前者は範囲導出 awk が fail-open で範囲を打ち切っても切り詰め後の範囲に残るため、**「範囲全体が検査されていること」を一切実証しない**。後者は打ち切られた範囲では定義が拾われず変異が検出されなくなるため、fail-open が起きたときに変異③が空振りする＝異常を露出させる |
...
**実測**（適用前 tree・ゲート A = `421-521` / ゲート B = `558-730`）: `containment_violations=0` / rc=0。
範囲を打ち切った場合（ゲート B を `558-700` と偽装）: `OUT-OF-RANGE gate B: TC-36 at L707 not in 558-700` / `containment_violations=1` / rc=1。**fail-open 経路を実際に検出することを確認済み**。
