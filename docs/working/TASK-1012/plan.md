...
| **T-04** | **変異検証 3 種**（1 つずつ入れて戻す）。①条件反転（**新規 2 ゲート限定**）→ AC-2 が FAIL ②ゲート B 終端を TC-36 手前へ → AC-1 が FAIL ③**ゲート外に、ゲート B 終端付近で定義される変数 `_t26_tgt36` を参照する 1 行を注入** → T-01 の越境検査（**AC-6**）が ≥1 件（C-2 R-002b） | `evidence/test-runs/t04-mutations.log` | agent | **高** | 🚩 3 変異すべてで期待 FAIL + 各復元後に再 PASS |
...
Mode を high-risk へ引き上げたことで、`.claude/rules/mode-classification.md` の**フェーズ適用マトリクス**上の適用が変わる。standard との実質差分は **V-2 が `-` → `○`** の 1 点である（V-3 は standard でも `○` で既に必須、V-4 は `critical` のみなので依然として適用外）。
