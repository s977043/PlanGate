...
> **判定の正本は `plan.md` の「Mode 判定」節**（定量 3 + 定性 4 = **7 軸**の表）。本節はその要約であり、齟齬がある場合は plan を正とする（C-1 R5 指摘 N-4: 改訂 3 まで本表が 5 軸のまま plan と食い違っていた）。
...
定量・定性とも最大が「中」→ **standard**。承認境界パス**外**のため例外ルールによる引き上げなし。
