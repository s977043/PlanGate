...
- [ ] **AC-6**: ゲート内で定義されゲート外から参照される shell 関数・変数が **0 件**である（`set -e` が無いため、越境があると子は `command not found` のまま継続 FAIL し、TC-13 の `0 failed` 判定を壊す）。検証は **TC-A6a**（静的）、検出力の実証は **TC-A6c**（変異③）
