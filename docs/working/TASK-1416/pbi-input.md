...
The proposed concept is **AI Execution Readiness**. It evaluates six dimensions but **does not create six new sources of truth**. It projects from existing Plan / evidence / verification / stop-and-replan information.
...
- Define Execution Readiness as a **projection over existing canonical artifacts**, not a parallel gate/state subsystem.
- Integrate the projection into existing Plan creation / C-1-C-2 review / ai-loop handoff surfaces.
...
- Define deterministic readiness outcomes:
  - `ready`
  - `needs_clarification`
  - `blocked`
- Define material-only representation so simple tasks are not forced to emit ceremonial N/A sections.
- Prepare fixed scenarios with one exact expected verdict each:
  - simple-ready
  - dependency-blocked
  - detection-missing
  - human-clarification
...
- Treating Execution Readiness as Human authorization; C-3 remains the authorization boundary.

## Readiness state semantics

### ready

Execution prerequisites are available, no Blocking Unknown remains, material failure modes are detectable, and re-plan / stop / escalation boundaries are sufficient for the task risk.

### needs_clarification

Execution cannot start because a **Human answer or decision** is required, and that answer can resolve the missing requirement / priority / risk-tolerance information without waiting for an external prerequisite.

### blocked

Execution cannot start because an **external prerequisite or required safety/evidence capability is unavailable**, including:
- required dependency unavailable;
- required evidence unavailable;
- material failure is not credibly detectable;
- required recovery / stop / escalation boundary cannot yet be established.

## 受入基準

- [ ] AC-01: Six AI Execution Readiness dimensions have one responsibility map and are projected from existing canonical sources rather than duplicated as new sources of truth.
...
- [ ] AC-11: Fixed scenarios assert exactly one readiness verdict each: ready / needs_clarification / blocked.
...
- [ ] AC-14: After upstream unblock, fresh inventory must finalize concrete files/commands/expected values and rerun C-1/C-2 **before** Human C-3; production tasks must depend on C-3 approval.
...
- Recovery / Escalation projects existing Replan Triggers / Stop Condition / Human Approval plus runtime owner references; it does not become another retry-policy source.
...
- Authorization risk: narrative C-3 stop wording without a matching `depends_on` edge.
...
- Which concrete production files and fixture paths remain after #1359 is implemented.
...
- Concrete production tasks must be regenerated after upstream dependencies are resolved; speculative placeholder paths are not sufficient for critical-mode C-3.
