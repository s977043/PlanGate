...
- resolution: current planning baselineからproduction tasksを除去し、T-13でPlan v2/todo v2を再生成してproduction task→H-01の物理依存を必須化。
...
- resolution: schema-valid verdictへ戻し、canonical 25 checksでplanning baselineを再評価。production Plan v2でもT-14で再実行必須。
...
- resolution: production tasksをbaselineから削除し、fresh inventory後のT-13でconcrete Plan v2を生成。placeholder 0をcompletion condition化。
...
Production Human C-3はT-15 independent C-2完了まで不可。
