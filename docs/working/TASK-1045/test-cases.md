...
`ta-25` の `222` 行 `if [ "$PG_T25_FOCUSED" = "0" ]` を境に **focused 群 / 通常群**が分かれる。
**mutation の kill 対象になる TC は focused 群（`222` 行より前）に置かなければならない。**

| TC | 配置 | 理由 |
|---|---|---|
| `T1045-TC-01`〜`06` | **focused 群（必須）** | `T1045-TC-09` / `TC-10` の mutation kill 対象 |
| `T1045-TC-07`〜`20` | 通常群 | mutation kill 対象ではない |
...
| ID | 対応 AC | 入力 / 前提 | 期待結果 |
|---|---|---|---|
| **T1045-TC-09** | AC-08 | **(a) 修正前へ戻す変異**: tmp 複製の guard で `# t1045-redirect-normalize` の正規化を **no-op 化**（= 旧 `grep -q '>'` 相当）。`PG_T25_GUARD` を mutant へ override して `ta-25` を focused 再実行 | アンカー `grep -c` == **1**、`sh -n` PASS、**`[FAIL] T1045-TC-01` が実出力に現れ子プロセス rc が非 0**、復元後 PASS |
| **T1045-TC-10** | AC-09 | **(b) 弱める側の変異**: tmp 複製の guard で `# t1045-file-redirect` の残存 `>` 判定を **常に false 化**。同 override | アンカー `grep -c` == **1**、`sh -n` PASS、**`[FAIL] T1045-TC-04` が実出力に現れ子プロセス rc が非 0**、復元後 PASS |
...
| AC-03（`>&2` fd 複製） | T1045-TC-03, T1045-TC-20 | 2 |
| AC-04（退行: `> <TOKEN>`） | T1045-TC-04, T1045-TC-12, T1045-TC-13, T1045-TC-14, T1045-TC-15, T1045-TC-19 | 6 |
...
- **ガードを弱める変更が 1 つも入っていない**（plan GC-1。AC-04〜07 + AC-09 で機械担保）
