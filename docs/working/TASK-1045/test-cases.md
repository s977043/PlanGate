...
| `T1045-TC-07`〜`TC-19` + `TC-21` + `TC-22`（**15 件**） | 通常群 | mutation kill 対象ではない。とくに `TC-07` は変異 (b) 適用下でも `copy-like` ルールで block され続けるため **kill 対象になり得ない** |
...
| ID | 対応 AC | 入力 / 前提 | 期待結果 |
|---|---|---|---|
| **T1045-TC-22** | AC-04（GC-8） | **`T1023-TC-05`（jq 不在 PATH）と同型**: `sed` を含まない一時 PATH（`cat` / `grep` / `sh` / `jq` のみを symlink）で guard を起動し、payload = `printf x > <TOKEN>` | **rc=2**（fail-closed）。**GC-8 の実装が無ければ `rc=0`（ALLOW）で落ちるため、この TC 自体が検出力を実証する**。**筆者が独立に実走再現済み**: 正規化が失敗する状態で真の書き込みが **ALLOW / stderr 出力なし / `set -e` でも中断しない** |
...
| AC-04（退行: `> <TOKEN>`） | T1045-TC-04, T1045-TC-12, T1045-TC-13, T1045-TC-14, T1045-TC-15, T1045-TC-19, **T1045-TC-22** | 7 |
...
- **TC 総数 22 / 対応 AC を持たない TC = 0（逆向き orphan も 0）**
  （内訳: focused 群 **7 件**（`TC-01`〜`06`, `TC-20`）+ 通常群 **15 件**（`TC-07`〜`TC-19`, `TC-21`, `TC-22`））
...
- **ガードを弱める変更が 1 つも入っていない**（plan GC-1。AC-04〜07 + AC-09 + **`T1045-TC-22`** で機械担保）
- **正規化ヘルパが fail-closed である**（plan GC-8。`sed` 不在 / 失敗で `rc=2`。`T1045-TC-22`）
