...
```text
T-00 ─┐
T-01 ─┴→ T-02 ─→ T-03 ─→ T-04 ─→ T-05 ─→ H-01(👤 Gate) ─→ T-06 ─→ T-08 ─→ T-09
...
| T-05 | `trusted_hash` **手順の文書化** | 可逆（文書のみ） | 不要 |
| **T-06** | **注記キー除去＝有効化** | 🔴 **不可逆点**（ここで初めて Codex に登録・発火しうる） | **必須** |
...
- [ ] **T-01**: fixture と baseline テストの導入
  ...
  - extras は **source される**（実行ではない）ため `pass` / `fail` を共有し、`pg_extra_contract_init "ta-65" …` で共有 exit 契約に載せる
...
- [ ] **T-06**: 注記キー除去＝**有効化**（🔴 **唯一の不可逆ステップ**）
  ...
  - **block が観測できなかった場合**: AC-07 を **WARN** とし、(a) 使用コマンドと `trustStatus` /
...
- [ ] **T-08**: 非回帰の証明
  ...
  - **基点を固定して**差分 0 を示す: `git diff $(git merge-base HEAD origin/main) -- scripts/hooks .claude`。
...
- [ ] **H-01（C-3 相当の追加ゲート / exec 中の有効化判断）**
  - depends_on: T-05
  - 判断事項:
    1. 有効化後の deny 範囲（plan の実測表 + T-01 で再現したログ）を許容するか
    2. **AC-07 の `codex exec` 実走 1 回（課金あり・`--dangerously-bypass-hook-trust` 無し）を許可するか**
    3. `CODEX_HOME` 側 `trusted_hash` 設定の適用（**外部状態の変更＝Human-owned**）
    4. **AC-07 が WARN で終わる可能性を承知のうえで進めるか**（U-4 未解決。bypass 無しでの発火は未実証）
...
- **前提条件 P-1（`hooks/list` の登録）が充足**（成果としては報告しない）
- **`sh tests/run-tests.sh` が全体 PASS**（出力に `TA-65` と `TA-15` が現れること）
