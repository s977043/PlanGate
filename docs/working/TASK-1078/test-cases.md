...
| AC-05 | 未知 rc は deny / reason 非空 / **出力が常に valid JSON** / **生成器失敗時も decision を失わない** | TC-11・TC-12・**TC-24**・**TC-25** + **全 deny TC の `json.loads` assertion** |
...
| **AC-11** | **hooks.json が Codex 受理形 / matcher に死に文字列なし / 宣言 stage と実体の drift 検出 / `hooks` サブツリーの改竄検出** | **TC-22a**（drift）・**TC-22b**（正規化ハッシュ + command 完全一致 + timeout 下限）・**TC-26**（導出同値性） |
| **AC-12** | **hook 解決のフォールバック** | **E-4 / E-5**（自動化・下記）+ **変異 7** |
...
#### 🔴 実走 env の固定（R4-F1 / **これが無いと「サンドボックスで PASS・本番で実効ゼロ」が作れる**）

**単体 TC 側では env 継承禁止を厳格に規定している（前提節・E-11 / E-12）のに、旧版は課金実走の env が未固定だった。**
既定 `STRICT=0` で deny するのは **(a) EH-3 の HO パス（`PLANGATE_HOOK_TASK` 未設定時のみ）** と
**(b) EH-9（`PLANGATE_DELEGATION_NOCOMMIT=1` 時のみ）** だけなので、operator は block を取るために env を能動的に整える必要がある。
そこで `PLANGATE_HOOK_STRICT=1` を与えれば EH-1 / EH-2 が広く deny し、**PASS を作れてしまう**。
だが plan 自身が「STRICT 既定 warning のため S-2 完了後も EH-1/2/6 は Codex で block しない」と書いており、
**サンドボックスで PASS・本番で実効ゼロ**が計画に違反せず成立する。

| env | 値 | 理由 |
|---|---|---|
| `PLANGATE_HOOK_STRICT` | 🔴 **未設定**（本番既定と同一） | **設定して取った block は AC-07 の根拠にできない** |
| `PLANGATE_BYPASS_HOOK` | **未設定** | escape hatch を無効化した状態で測る |
| `PLANGATE_HOOK_TASK` / `PLANGATE_DELEGATION_NOCOMMIT` | **下記ケースごとに固定**（`env -u` で明示 unset） | 実運用に実在する条件でのみ PASS を認める |

**deny 対象は次のいずれかに固定する**（どちらを使ったかを evidence に明記）:

| ケース | env | 操作 | deny する hook | 実運用での実在性 |
|---|---|---|---|---|
| **α（推奨）** | `PLANGATE_DELEGATION_NOCOMMIT=1` / `PLANGATE_HOOK_TASK` は**設定あり**（委譲セッションの実態に合わせる） | `Bash` で `git commit -m wip` | **EH-9** | **高**。委譲セッションは実際にこの env を持つ |
| **β** | `PLANGATE_HOOK_TASK` **未設定** / 他は未設定 | `apply_patch` で `.claude/settings.json` | **EH-3（HO）** | **限定的**。**TASK 文脈を持つセッションでは #1089 により発火しない**ため、採用時はこの限界を必ず併記 |

- 🔴 **`PLANGATE_HOOK_STRICT` を上げないと block が取れない場合、AC-07 は WARN**（＝終端 B）。**STRICT を上げて PASS にしない。**
- **AC-07 = PASS は「常に block する」ことを意味しない。**「**上表の運用条件で block する**」ことを意味する。
  この条件は **doc（AC-09 (h)）に必ず列挙**する。
- **使用した env 一式（設定値と unset の別）を evidence に逐語で残す。**

- 手順: `codex exec` を 1 回
  - 🔴 **`--dangerously-bypass-hook-trust` を付けない**（付けた run の結果は AC-07 の根拠にできない）
  - 🔴 **上表の env を明示設定する**（セッションからの継承禁止。unset は `env -u` で明示）
...
- 🔴 **判定の帰結**:
  - **PASS** → `todo T-06`（不可逆な有効化）に進んでよい（**終端 A1 または A2**。実リポジトリでの確認実走 `T-06b` を行えば A1）
  - **WARN** → 🔴 **`todo T-06` を実行しない**。kill switch を保持したまま完了する（**終端 B**）
- 🔴 **備考（主張の強度 / R4-F3）**: 未発火を「**U-4 が否定側に確定した**」と書いてはならない。**過剰主張である。**
  サンドボックスで発火しなかった原因が「**手書き `[hooks.state]` による trust が実行時に効かない**」（evidence L109）だった場合、
  **それは実リポジトリでの非発火を含意しない**（trust 付与経路が異なりうる）。
  正しい記述は「**サンドボックス条件下では未発火。U-4 は依然として未解決（trust 付与経路の差が残る）**」。
- 🔴 **同値性の非対称（Model A 指摘）**: サンドボックスの同値性論証は **PASS 方向にしか効かない**。
  PASS なら「同一 env・同一 bridge・同一 hooks.json 内容で block する」ことが言えるが、
  WARN は「実リポジトリでも発火しない」ことの証明にならない。**doc と handoff でこの非対称を明記する。**
...
### TC-25: シリアライザが使えない環境でも decision が失われない（R4-F5）

- 前提: **`python3` を PATH から外す**（または出力生成コマンドを強制的に rc≠0 にする）
- 入力:
  1. deny すべき payload（例: TC-03 と同じ HO パス）
  2. allow すべき payload（例: TC-05 と同じ通常作業）
- 期待:
  1. deny ケース: **`permissionDecision="deny"`** かつ **stdout が `json.loads` できる**（reason は固定文言でよい）
  2. allow ケース: **`permissionDecision="allow"`**（**全 deny 化していない**）
  3. **どちらのケースでも stdout が空にならない**
- 種別: Unit（自動）
- 🔴 **本 TC が無いと、R-12（reason の非エスケープ）の是正が「python3 依存の全 allow 経路」を新設したことに気づけない**。
  退避経路は **reason を補間しない静的リテラル**であること（補間する fallback は R-12 の穴の再導入）

### TC-26: 導出変換がサンドボックスと実リポジトリの同値性を保証する（R4-F2）

- 手順:
  1. 導出スクリプトに **実リポジトリの `.codex/hooks.json`** を入力し、有効化後の目標形を生成する
  2. **`diff <(導出結果) <sandbox>/.codex/hooks.json` が空**であることを assert する
- 期待: 差分 0
- 種別: Unit（自動 / T-05b の合否判定に組み込む）
- 🔴 **evidence 添付ではなく assertion にする**。旧版は「同一内容にする」と書くだけで、
  **同値性を検証する機械が存在しなかった**（実測: TC-22a 節にサンドボックスへの言及 **0 件**）
- 🔴 **導出は「未知キー除去」だけでは足りない**。実測で実ファイルの top-level は `['$schema_note', '$note', 'hooks']` であり、
  **除去のみでは `{hooks}`** になる。変換規則は **(i) `$` 始まりの top-level キーを除去 (ii) 内容を `description` に統合 (iii) `hooks` は無改変**
- **`todo T-06` も同じスクリプトの出力で置き換える**（手編集しない）ため、検証した形と本番に入る形が**同一生成物**になる
...
- 手順（課金ゼロ・静的）:
  ...
  4. さらに宣言 stage 別に assert する:
     - **`enabled`**: 🔴 **未知の top-level キーが 1 つも無い**（`{description, hooks}` の部分集合。`description` の有無は問わない）
     - **`disabled`**: **未知キーが実在する**（＝ kill switch が効いている）かつ想定どおりのキー（`$schema_note` / `$note` 等）
  5. **どちらでも 5 hook の記述が存在する**ことを assert する（stage 非依存の不変条件）

> ⚠️ **`enabled` を `{description, hooks}` への「厳密一致」にしない**（R4-F2）。
> 実測で **実ファイルの top-level は `['$schema_note', '$note', 'hooks']`** であり、
> **未知キーを除去しただけでは `{hooks}`** になる（`description` が無い）。
> Codex の受理条件は「**未知の top-level キーが無いこと**」なので、assertion もそれに合わせる。
> `description` へ注記を寄せるのは**リポジトリの慣行**であり、導出変換（plan の Task 5a-2）が担う。
...
#### TC-22b: `hooks` サブツリーの**正規化ハッシュ固定**（**T-04 のコミットで導入**）

> 🔴 **宣言 stage 方式（TC-22a）だけでは `command` 改竄で迂回できる**（R4-F4）。
> TC-22a が見るのは top-level のキー集合と「5 hook の記述の存在」だけなので、以下は**すべて GREEN のまま強制力を消せる**:
>
> | # | 迂回経路 | TC-22a で防げるか |
> |---|---|---|
> | 1 | **`command` 文字列の改竄**（`sh -c true` / 別パスへ差し替え） | ❌ |
> | 2 | **`timeout: 0`** の悪用 | ❌ |
> | 3 | hook 配列内へのキー追加・差し替え | ❌ |
> | 4 | **テストファイル自体の削除・rename** | ❌ |
>
> `.codex/**` は **AI-owned・HO 対象外・技術層の強制なし**（Task 6 で確定）なので、**1〜4 はすべて AI が単独で実行できる**。

- 手順（課金ゼロ・静的）:
  1. `.codex/hooks.json` の **`hooks` サブツリーのみ**を **正規化（キーをソートした JSON）** して **SHA-256 を計算**する
  2. **テスト内に宣言したハッシュ定数と一致すること**を assert する
  3. 可読な失敗メッセージのため、以下も個別に assert する:
     - matcher 文字列に **`Edit` / `Write`** が含まれない
     - 5 つの `command` が **期待文字列と完全一致**する
     - 各 `timeout` が **1 以上**（`timeout: 0` の封じ）
- 期待: ハッシュ一致 + 上記 3 点成立
- 種別: Unit（自動 / 静的）
- 🔴 **これで迂回経路 1・2・3 が塞がる**。`hooks` サブツリーへの**いかなる変更もハッシュを変える**ため、
  **同一 diff 内でハッシュ定数の更新が必要**になり、**レビュー可能面に必ず現れる**（本リポジトリの `plan_hash` と同じ設計）
- 🔴 **経路 4（テスト自体の削除）は塞げない**。恒常 CI assertion が必要であり、**S-3（doctor / CI 化）の範囲**。
  **doc の限界に明記する**（AC-09）
- **stage 非依存**: `todo T-06` は **top-level しか変えない**ため、`hooks` サブツリーのハッシュは**有効化の前後で不変**。
  ハッシュ更新が要るのは **T-04（matcher 変更）のときだけ**であり、**T-04 が本 TC の所有者**である理由もこれ

**採用理由（R4-F4 の選択）**: 「`command` の完全一致 + `timeout` 下限」だけでも経路 1・2 は塞げるが、
**経路 3（配列内のキー追加・差し替え）が残る**。正規化ハッシュなら **1・2・3 を 1 つの不変条件で覆える**うえ、
**将来 hook を増減したときも必ず明示更新を強制**できる。保守コスト（正当な変更のたびにハッシュ更新）は
**意図した摩擦**であり、本リポジトリが `plan_hash` で既に採用している型である。
...
| Unit（静的） | **TC-22a**（T-01 所有・drift 検出）・**TC-22b**（T-04 所有・正規化ハッシュ）・**TC-26**（T-05b 所有・導出同値性） | ✅ 完全自動・課金ゼロ |
| Unit（環境依存） | **TC-25**（python3 不在時の decision 保存） | ✅ 完全自動・課金ゼロ |
| Mutation | TC-08・TC-09・**TC-21**・**TC-23（4a / 4b）**・**変異 5（TC-24）**・**変異 6（TC-25）**・**変異 7（E-4 / AC-12）** | ⚠️ 変異適用は手動、判定は自動 |
