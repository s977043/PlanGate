...
| A-12 | `delivery/record.jsonl`（Delivery 実行記録） | **なし** | **未対応** | `grep -n "issued_by\|actor\|principal\|session_id\|requested_by\|decided_by" scripts/ai-loop/{delivery,collector,executor,reconciler}.py` → **0 件**。entry 4 種は `intent` / `receipt` / `state` / `merge_ready`（`delivery.py:407,417,426,589`）で、フィールドは `action_id` / `action_kind` / `payload` / `state` / `head_sha` / `pr_number` / `reasons` / `at` / `entry_id` のみ（契約: `docs/workflows/ai-loop/delivery-state-machine.md:83-96`）。**「誰が intent を出し、誰が receipt を書いたか」は記録されない** |
...
| A-17 | Delegation（委譲）の概念 | **なし** | **未対応** | `delegation` / `allowed_actions` / `valid_until` 相当のフィールドは schema・実装のいずれにも不在。既存の「委譲」語彙は `docs/ai/subagent-delegation/`（サブエージェント派遣プロンプトの**規範**）であり、機械的な権限委譲 record ではない |
...
| A-23 | 昇格経路の入口条件（署名の位置づけ） | **既存で満たす（要件として定義済み）** | `docs/ai/ai-loop/hotl-merge-entry-criteria.md:44-48`「**条件 1: provenance 発行元検証（署名 / HMAC）** — `issued_by`（誰が承認したか）の自己申告を解消し、署名 / HMAC 等で発行元の真正性を検証可能にする」/ `:95` は「auto-approve 経路が『別アカウントでの自己承認』と機能的に等価にならないこと」を要求。→ **#980 Phase 5 / 6 はこの既存 entry criteria の充足手段であり、新しい要求ではない** |

### 集計

全 23 項目（A-1〜A-23）の内訳。**A-2 と A-16 は同一項目内で判定が割れる**ため「混合」に分離し、二重計上しない（合計 2 + 12 + 7 + 2 = 23）。

| 判定 | 件数 | 項目 |
|---|---|---|
| 既存で満たす | **2** | A-18 規範層（sockpuppet 禁止 / merge = Human-owned）/ A-23 昇格入口条件の定義（hotl 条件 1） |
| 一部満たす | **12** | A-1 実行主体 / A-6 実運用量 / A-7 `approved_by` / A-8 presence L1-L4 / A-9 C-4 / A-10 c3-prime `issued_by` / A-11 arbiter provenance / A-13 `decision-log.jsonl` / A-15 skip-decision-log / A-19 maker-checker / A-20 W チェック / A-22 自己承認ガード機械層 |
| 未対応 | **7** | A-3 追加キー不可（`additionalProperties: false`）/ A-4 event 語彙 / A-5 CI 強制 / A-12 Delivery record の actor / A-14 hook 監査ログ（非追跡・actor なし）/ A-17 Delegation / A-21 発行元真正性 |
| 混合（定義あり・利用なし等） | **2** | A-2 `agent` / `by` / `plan_hash`（schema 定義済み・writer 未使用）/ A-16 metrics event schema（設計先例としては満たす・格納先としては使えない） |

> **本 PBI の実装対象は「未対応」7 件と「一部満たす」のうち主体軸に関わるもの**に限定する。A-18（規範層）と A-23（entry criteria）は**再定義しない**（既存正本を参照する）。A-22 の機械層の穴は **#928 の scope**（本 PBI は policy 層の宣言まで、ガード実装は #928 に残す）。
...
| S-6 | **Principal / ActorSession / Delegation の最小 schema 定義** | Phase 1 | 新規 schema または既存 schema の additive 拡張（配置は U-2） |
| S-7 | **append-only Audit Event の最小 schema 定義**（`schema_version` / `correlation_id` / `causation_id` / `idempotency_key` / policy・model・tool・harness の version または digest を含む） | Phase 1 | 同上 |
...
| **AC-2** | Principal、Role、ActorSession、Delegation が区別されている | Phase 1 | 4 概念が**別の型 / 別のフィールド群**として定義され、ADR に「同一 Principal が複数 Role・複数 ActorSession を持ちうる」ことが明記されている |
...
| **AC-5** | Executor による自己レビュー・自己承認を policy で禁止できる | **Phase 3**（本 PBI は語彙まで） | 本 PBI では独立性レベル（Level 0〜4）の語彙と比較対象フィールド（`principal_id` / `actor_session_id` / `parent_session_id` / `delegation_id` / `provider` / `model_id`）が schema 上比較可能であることまで。**禁止の実装は Phase 3 / 機械層ガードは #928** |
...
| **AC-P2**（起案） | **#981 との重複が発生していない** | Phase 0 | ADR / schema に `plan_hash` / `plan_package_hash` / `artifact_hashes` の**算出規則・検証規則の再定義がない**ことを確認。Plan 側は `plan_ref` / `approval_ref` として参照のみであることが ADR に明記されている |
...
| 候補 | HO 接触 | CI 強制 | 追跡（git） | 評価 |
|---|---|---|---|---|
| ① `run.ndjson` の既存未使用プロパティ（`agent` / `by` / `plan_hash`）を使う | **なし** | **なし**（A-5） | あり | **最小差分**。ただし `by` は 1 個の文字列であり Principal / ActorSession / Role の 3 概念を表現できない。`schema_version` も無い（`run-event.schema.json` の `required` は 4 キーのみ）→ **Phase 1 の受け皿としては不足** |
| ② `run-event.schema.json` を additive 拡張（新キー + `^_` patternProperties 追加） | **あり**（`schemas/*.schema.json` は HO 対象） | なし（A-5） | あり | schema を直せば表現力は足りるが、**HO patch 運用（AI は patch 提示まで・適用は Human-owned）**になる。かつ CI 強制が無いままなので「schema に書いたが検証されない」状態が残る |
| ③ 新規 sidecar schema + `docs/working/TASK-XXXX/audit/*.json` | **あり**（新規 `schemas/*.schema.json` 作成も HO 対象） | **あり**（`*.json` は `validate-schemas.py` の rglob と `schema_mapping.py` 登録で検証可能） | あり | 新規 schema の追加は HO だが、**CI 強制を得られる唯一の経路**。`.ndjson` を選ぶと A-5 のとおり CI から外れる |
| ④ `delivery/record.jsonl` へ actor 系 entry を additive 追加 | なし（`scripts/ai-loop/**` は HO 外だが **rollout-policy §2 carve-out で escalate 固定**） | なし | あり | 既存の append-only + `entry_id` 冪等機構（`delivery.py:476-487`）と `RecordError` fail-closed（`:450-470`）を**そのまま再利用できる**。ただし ai-loop Delivery 経路に閉じ、ai-dev 経路（`run.ndjson`）を覆わない |
| ⑤ metrics `events.ndjson`（`plangate-event.schema.json`） | あり | なし（設計上 `plangate metrics --validate` で検査） | **なし**（`.gitignore:56`） | `schema_version` 必須 + `parent_event_id`（causation）の**設計先例**として参照価値が高いが、**追跡対象外**かつ `by` に人名不可の privacy 制約があるため監査正本にはできない |
...
| **rollout-policy §2 判定基盤 carve-out** | `scripts/ai-loop/**` / `docs/workflows/ai-loop/**` に触れる場合、[`rollout-policy.md`](../../workflows/ai-loop/rollout-policy.md) §2 の判定基盤 carve-out に該当し **escalate 固定**。**これは規範層**であり、`arbiter.py` の `boundary_check` は `ho-paths.md` の HO 表からのみ touches-HO を導出するため当該パスは **`boundary=clean` と機械判定される** → **実行者が escalate する責務を負う**（同 §2 の「規範層である旨の明示」） |
...
- **U-2**: **Audit Event の物理的な格納先**（上記候補比較表の ①〜⑤）。判断軸は (a) HO 接触の有無、(b) CI 強制の有無（`.json` か `.ndjson` かで `validate-schemas.py` の rglob 対象が変わる — A-5）、(c) ai-dev / ai-loop の両経路を覆えるか、(d) git 追跡対象か（A-14 / A-16 は非追跡）、(e) 既存の冪等 append 機構（`delivery.py` の `entry_id`）を再利用できるか
