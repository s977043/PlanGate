...
| **`/bin/bash`（非 POSIX モード）** | **継続（`DIAG` / `AFTER` とも出る。rc=0）** |

- **終了するか否かは「POSIX 準拠モードか」で決まり、`set -e` の有無では決まらない。**
  `.` は special built-in であり、その失敗は **POSIX 準拠モード**（`dash` / `sh` として起動した
  bash）では**それ自体が非対話シェルの終了要因**になる。一方 **`bash` として起動した非 POSIX
  モードでは終了しない**（上表の実測）
- したがって前版の「`set -eu` 下で `.` が失敗すると」は**原因の帰属が誤り**であり、
  さらに「`set -e` の有無に依らない」と言い切るのも**過度な一般化**である
  （**C-1 R7 / MN-O** で検出。R-021 と同じ「シェル挙動の過度な一般化」が是正文中に再発した）
- **ただし結論は 4 シェルすべてで変わらない**。CI（dash）でも `sh` 実行の bash でも
  診断に到達しない以上、**source する前に確認する形しか成立しない**。設計・AC・TC は不変
