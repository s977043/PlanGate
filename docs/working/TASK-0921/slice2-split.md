...
## 4. 管理方式（同一 TASK 内後続スライス vs 別 TASK 起票）— 両案比較・**Human 判断待ち（G-1）**

plan.md L338-339 は「**Slice 2 は本 PBI のスコープに含めたまま『後続スライス』として扱う。別 PBI へ
切り出すか否かは Slice 1 完了時に判断し、本時点では決めない**」と明記する。すなわち **plan は管理
方式を確定しておらず、Slice 1 完了時（= 今）の判断事項として Human に留保している**。2026-08-12 の
D-5 裁定は「3 分割・C-3 サブスライスごと発行」を確定したが、**同一 TASK か別 TASK かは明示していない**。
指示（曖昧な場合は両案比較を作って停止）に従い、以下を提示して**確定せず停止する**。
...
**ワーカー所見（確定ではない）**: C-3 をサブスライスごとに発行する裁定を既存機構（plan_hash / EH-3 /
`bin/plangate exec`）の上で満たすには**案 B が構造的に整合**する。ただし決定権は Human にある。

## 5. Human 判断待ち一覧（本書の出口）

| ID | 判断事項 | 選択肢 |
|---|---|---|
| **G-1** | 管理方式 | 案 A（同一 TASK 内）/ 案 B（新 TASK ×3 起票）— §4 |
| **G-2** | Mode 定量算入の扱い | ta-61 縮約・writeback を定量算入するか（各サブスライス C-3 で確定）— §3.1 |
| **G-3** | 本分割案（SS-2-1 / 2-2 / 2-3 の割当・順序）の承認 | 承認 / 修正指示 — §3 |
| （既存） | TASK-1044 の C-3 | 全サブスライス exec の前提 blocker — §2 |
