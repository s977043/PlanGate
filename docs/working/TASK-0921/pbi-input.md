...
| **層 A** | ROOT / FIXTURES 解決の fallback を持ち、**standalone でもテスト本体が走る**（構造述語で判別 — 判定コマンド C） | **12** | #914 移行の 11 本（`ta-39` / `ta-43` / `ta-44` / `ta-45` / `ta-46` / `ta-47` / `ta-49` / `ta-50` / `ta-51` / `ta-52` / `ta-53`）+ **`ta-40`**（初版になし。`ta-40:7-10` が `schemas/c3-approval.schema.json` の不在を見て root を 1 段上げる自己補正 fallback を持つ） | 正しく実行され FAIL を数えるが **exit 0**。**加えて 12 本全数が `pass`/`fail` を初期化していない**（下記 A-1'）。**うち 3 本は早期 `exit 0` 経路を持つ**（下記 A-2'） | **`ta-26` パターンの複製 = フラグ + カウンタ初期化 + 末尾ブロックの 3 点を 12 本全数へ**。source 経路では exit しない。早期 exit 経路は別途 rc 設計（MJ-4） |
...
### A-2'（MJ-4）: 層 A の 3 本は**末尾ブロック追加だけでは実害経路が塞がらない**
...
1. **層 A（12 本）**: `ta-26` パターンを全数へ適用 = **standalone フラグ + `pass`/`fail` の自前初期化 + 末尾サマリ + `fail != 0 → exit 1` の 3 点**（A-1' により **12 本全数で初期化が必要**）。加えて **早期 `exit 0` 経路を持つ `ta-39` / `ta-43` / `ta-44` は rc 設計（前提未充足 = exit 3）**（A-2'）
...
- **AC-2**: **層 A 12 本の各ファイル**について、以下を負側テストで固定する:
  ...
  - (c) **【MJ-4 で追加】前提未充足 SKIP 状態での rc** — 早期 `exit 0` 経路を持つ `ta-39` / `ta-43` / `ta-44` について、**前提（apply 未実行）を満たさない状態で standalone 実行したときの rc が「検査していない」を表明する**（例: exit 3）。**rc=0 は不可**。これを満たさない限り実害経路（`apply-eh3-doc-light.sh:68` → `ta-39`）は塞がらない
...
| タスク数（見込み・定量） | **16-24**（初版想定 12-20 から **+4**）。内訳の増分: **① 層 A 12 本のカウンタ初期化**（A-1' — 「末尾ブロックのみ」ではなくなった）/ **② 早期 exit 3 本の rc 設計 + AC-2 (c) の負側テスト**（A-2'）/ **③ 判定コマンド B の注入位置汎用化**（MJ-3 — 現状 4 本にしか効かない）/ **④ 正規述語への移行と allowlist 廃棄 DoD**（MJ-6） | **high-risk 上限〜critical**（一括なら 21+ = critical 確定） |
...
>
> **River Review 反映による見積り変化（MJ-2 / MJ-4）**: タスク数は **12-20 → 16-24**、定性「変更種別」は **standard 相当 → high-risk**（exit 3 の意味レイヤーが新規設計）、Slice 2 以降の総量は **36 → 41 本**（D-2 (c) 採用時）。**Slice 1 の 14 本という規模は変わらないが、1 本あたりの変更量が「末尾ブロック追加」から「フラグ + 初期化 + 末尾ブロック（+ 3 本は rc 設計）」へ増える**。判定案自体（Slice 1 = high-risk / 一括 = critical）は維持。
