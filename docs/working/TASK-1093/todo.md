...
> mode=**high-risk**（**v3 で critical から戻した** / C-3 2026-08-18 の案 B。**タスク数は v5 実測 24**）。
...
>
> **v5（C-3 裁定 2026-08-18 round 2 反映 / U-6 不採用）** — **T-08 / T-08b（凍結リスト + `legacy`）を除去**し、
> **T-08d（`n/a (local)` の実行時根拠検査）を新設**。Agent タスクは **実測 24 本**。
...
| T-02 | 各 script の**冪等判定の所在**を実測して記録（**書き写さない**。「判定を持つ / 持たない」を記録）。**是正はしない（#1114）** | agent | T-01 | 不要（読取のみ） | 判定を持たない script を名指しで列挙（例: `apply-task-0134-progress.sh` は引数解析すら無い）。**是正対象の特定は #1114 へ渡す** |
...
| ID | タスク | Owner | depends_on | rollback | 🚩 |
|----|-------|-------|-----------|----------|----|
| T-04 | `scripts/apply-registry.tsv` 作成（列 = `script` / `scope` / `targets` / `defer` の **4 列**） | agent | T-03 | `git rm scripts/apply-registry.tsv` | **`probe_expr` 相当の列が存在しない**こと（v1 方式への逆戻り検出）。**`contract` 列も存在しない**こと（**U-6 不採用** / §3-bis）。`scope=local` の `targets` に tracked が 1 つも無いこと。**`defer` 列は空で作る**（AI は defer を発行しない / SC-2） |
...
| T-08c | **検査不能時の fail-closed 実装**（§3-ter / **最優先**）: issue state 取得不能（`defer`）/ `targets` の tracked 判定不能（`n/a (local)`）をすべて `undecidable` へ | agent | T-06 | `git checkout origin/main -- scripts/release-prep.sh` | **「取得失敗 → 免除」経路が存在しない**（**SC-8**）。offline / 非 git ディレクトリで当該行が NG になることを**実走確認** |
| T-08d | **`n/a (local)` の実行時根拠検査**（§3-ter-2 / **v5 で新設**）: `targets` の tracked 判定を実行時に評価 | agent | T-08c | 同上 | `scope=local` の**自己申告だけで `n/a` を与えない**。tracked が 1 つでもあれば `undecidable`（TC-35）、判定不能でも `undecidable`（TC-36） |
...
| T-16 | **MUT-6'（判定品質の kill / fixture 版）**: sandbox の**契約準拠 fixture apply script** の実装本体を壊し marker/コメントは残す変異で、`--dry-run` が **rc=10 に反転**し検出器が `pending` を出すことを要求 | agent | T-14 | 不要（sandbox 内） | kill されなければ**緑にしない**（**SC-6**）。**実 script 全数に対する MUT-6 は #1114**（実 script が契約適合して初めて実行可能） |
| T-17 | MUT-1〜5（検出器側の call site を壊す変異）で kill を実証 | agent | T-14 | 不要（sandbox 内） | 空振り fixture でないこと |
| T-18 | AC-5: (i) `.claude/settings.json` 有 / 無 の 2 sandbox で出力**完全一致** + (ii) **単調安全性**（ネットワーク有無 × git repo 有無 の 4 環境・**比較可能な 5 ペア**で NG→OK が 0 件 / §3-quater） | agent | T-14 | 不要（sandbox 内） | worktree 実機 + 通常 checkout 実機でも各 1 回実走して証跡化。**比較不能ペア（network のみ / git のみ）は比較しない** |
...
| ID | タスク | Owner | depends_on | rollback | 🚩 |
|----|-------|-------|-----------|----------|----|
| T-21 | follow-up issue 起票: (i) **`ack` / `defer` の hook 層保護**（HO 定義本体の変更が必要・**C-3 で「本 PBI では塞げない」と確定**）/ (ii) `--check` の CI 配線（HO・Human・**U-5 持ち越し**） | agent | T-20 | issue close | **塞げないことを黙らない**（**R-004 / U-5**）。#1114 と相互リンク |
| T-21b | **#1114 の AC と本 PBI の引き継ぎ内容の突合**（**起票ではなく既存 issue の受入条件の確認**） | agent | T-20 | — | **#1114 は起票済で AC-1〜AC-7 が付与済**（2026-08-18）。`TC-17` 相当 = #1114 AC-2 / `AC-3` 実 script 実証 = #1114 AC-3 / `MUT-6` 実 script 全数 = #1114 AC-4 / 台帳の全 `scope=release` 行が確定状態になること = #1114 AC-1 に**受理済であることを確認**する。**齟齬があれば #1114 側を是正**（本 PBI 側で抱え直さない） |
| T-22 | `handoff.md` 発行（必須 6 要素） | agent | T-21 | — | 既知課題に「`defer` の hook 未保護（follow-up）」「`--check` CI 未配線（U-5）」「**契約非適合 script が `undecidable` として残り `--check` が #1114 完了まで NOT READY になる（仕様 / §3-bis）**」を列挙 |

## 👤 Human タスク

| ID | タスク | Owner | タイミング | 依存 |
|----|-------|-------|-----------|------|
| **H-1** | **C-3 ゲート（人間必須 / mode=high-risk）**: 残る判断は **U-7**（V-4 の要否）のみ。**U-1 / U-4 は 2026-08-18 に裁定済 / U-5 は持ち越し確定 / U-6 は round 2 で不採用確定** | human | exec 前 | C-1（**`review-self-5.md`**）+ C-2（`review-external.md`）+ [C-3 裁定](https://github.com/s977043/PlanGate/issues/1093#issuecomment-5320820417) + ai-loop run-033（round 1 / 2） |
| **H-1b** | **承認レコードの発行**（**v3 / v4 / v5 と plan が変わっている**ため、**本更新の後**に発行する） | human | H-1 の後 | H-1 |
...
- **H-1/H-1b → T-04 以降**: 承認レコードの発行前に進まない（**U-6 は不採用確定なので分岐は無い**）
...
- **`--check` を緑にするための免除列・除外条件を足さない** — SC-9（緑にする道は **#1114 の完了**のみ）
