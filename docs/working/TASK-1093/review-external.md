...
|-------|----------|------|--------|--------------|-------|
| R-001 | major | 設計妥当性 | **accepted** | plan v2「方式変更の理由」「Approach 3/4」/ todo T-04・T-09 / TC-21・E-05 | **方式変更で解消**。台帳は判定を持たないため `probe_kind` 不要（`cmp -s` 型 4 本は script 自身の判定をそのまま使う）。`pending`+`defer` の出口を新設。**`undecidable` には `defer` を効かせない**（握りつぶす経路を作らない）＝ TC-21 |
| R-002 | major | コードベース整合 | **accepted** | plan v2「方式変更の理由」/ Testing Strategy「判定品質」/ todo T-16 / **TC-16・TC-17・MUT-6** | **方式変更で根本対応**。marker probe を**廃止**し、判定を script 内の 1 箇所（`--apply` と同一パス）に一本化。加えて **MUT-6**（実装本体を壊し marker を残す変異）と **TC-17**（負の対照を集合で定義）を追加 |
| R-003 | major | 設計妥当性 | **accepted** | plan v2 Approach 1/2 / **TC-18・TC-19・E-09・E-10** / MUT-1 | **「script を実行しない」の主張を取り下げ**（推奨対応 2 を採用）。`PLANGATE-APPLY-STATUS` の stdout cross-check は**廃止**。実行ガード（rc 捕捉 / timeout / 想定外 rc → `undecidable`）を仕様化し TC を 3 本追加（rc その他 / 出力なし / timeout） |
| R-004 | major | 設計妥当性 | **partially accepted** | plan v2 Approach 4「`defer` の発行元・保護」/ todo T-09・T-21 / **TC-20・TC-21・TC-22** | TC 2 本要求 → **3 本**追加。`decision-log` 1:1 + **OPEN issue 検査**も採用。**hook 層保護は採用できない** — HO 9 カテゴリは `check-plan-hash.sh` 側で固定され、追加には HO パス（hook / workflow）変更が必要で本 PBI では不可能。**塞げないことを明記して follow-up 起票**（T-21）+ handoff 既知課題に残す |
| R-005 | major | コードベース整合 | **accepted** | todo T-14 🚩 / **TC-24** / plan Constraints | ta-61 契約（marker 1 個 / `pg_extra_contract_init` basename・capability 一致 / `finalize` / rc 層 0-3 / standalone 両対応 / 7 env unset / `register_cleanup`）を 🚩 に明記。`_pending_migration` に**入れない**（新規は初日から full 準拠）ことを TC-24 で検査 |
| R-006 | minor | コードベース整合 | **accepted** | plan v2 Approach 5 / todo T-12 / **TC-23・MUT-7** | scope 内として対応。`run_checks \|\| true` を撤廃し `--check` と同じ rc 伝播に揃える |
| R-007 | minor | 設計妥当性 | **accepted** | **TC-12 を改訂** / plan Files 表 | TC-12 の負の主張を「`run_checks()` 本体に `sync-plugin-installed` 参照が 0 件」（実際に変化する対象）へ差し替え。**runbook テンプレ更新は Non-goal に明記**（別 PBI） |
| R-008 | minor | コードベース整合 | **accepted** | plan v2 verdict 表 / todo T-10 / **TC-09** | `n/a (local)` 行に `bin/plangate doctor --check-settings` への導線を表示。TC-09 の期待に含めた |
| R-009 | minor | コードベース整合 | **accepted** | plan v2 Approach 6 / todo T-05 🚩 | 追記ではなく**既存記述と同時に整理**し rc 規約を 1 か所へ統合することを Step 4 / T-05 に明記 |
| R-010 | minor | 設計妥当性 | **accepted** | plan Testing Strategy「sandbox コスト」/ todo T-15 / test-cases「sandbox コスト方針」 | `docs/`（18M）を除外した最小サブツリー + 1 回複製の使い回し。超過時は MUT-6 を別 job へ退避すると明記 |
| R-011 | info | 設計妥当性 | **accepted** | plan **U-1** / todo H-1 | U-1 に `apply-ai-loop-workflow-command.sh` を**名指しで**含めた（適用すると 2 週間分の退行するため `defer` 対象候補） |
| R-012 | info | 設計妥当性 | **accepted** | plan Step 1 🚩 / todo T-01 🚩 / TC-14 | 🚩 を「`comm -3` の集合同値」に統一。「34」は evidence 内の**測定値**としてのみ残す |
| R-013 | info | コードベース整合 | rejected（レビュア自身が反証済） | — | 対応不要。v2 は `--dry-run` のみ実行し `--apply` は実行しない点も不変 |

### 反映コミット

- `Refs: R-001 R-002 R-003 R-004 R-005 R-006 R-007 R-008 R-009 R-010 R-011 R-012`
- 反映は **1 回のみ**（working-context.md の順序規約）。以降の変更は C-3 承認後まで行わない。

### 反映にあたって行った設計判断（要 C-3 確認）

1. **方式を維持せず変更した**。R-001 / R-002 / R-003 はいずれも「台帳に判定を**書き写す**」ことに
   起因しており、marker を増やす小手先の修正では同型が残ると判断した。
   v2 は **script 自身の冪等判定（`--apply` と同一パス）を exit code 契約で読む**。
2. その結果 **`scope=release` の apply script（31 本前後）に契約適合の改修が必要**となり、
   **Mode が high-risk → critical に上がった**。スコープ分割の是非を **U-4** として C-3 に上げる。
3. **`--check` が CI に一度も配線されていない**（`grep -rn "release-prep" .github/` → 0 件）ことを
   plan の Non-goals に明記した（`.github/workflows/*` は HO のため AI は配線できない）。**U-5**。
