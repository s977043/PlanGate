...
> 配置想定: `docs/workflows/ai-loop/stop-rollback.md`
...
- 本書は以下の既存正本の**再定義ではない**。値・機構の変更が必要な場合は各正本の版上げ手続きに従う
  （[`loop-safety-gates.md`](../../workflows/ai-loop/loop-safety-gates.md) §6 と同じ扱い）:
  - [`decision-table.md`](../../workflows/ai-loop/decision-table.md) §3（priority 0〜6）/ §6（CB-1〜3）
  - [`arbiter-policy.md`](../../ai/ai-loop/arbiter-policy.md) §7（escalate 予算）
  - [`00_concept.md`](../../workflows/ai-loop/00_concept.md)（対応ラウンド上限 3）
  - [`loop-safety-gates.md`](../../workflows/ai-loop/loop-safety-gates.md)（Gate 1〜5）
...
| discovery 候補ゼロ終端 | `scripts/ai-loop/discovery.py:296-305` | 全 issue が除外 | exit 0・候補なしと明示（正常終了） |
...
- [`docs/workflows/ai-loop/decision-table.md`](../../workflows/ai-loop/decision-table.md) §3 priority 0〜6 / §5 provenance / §6 サーキットブレーカー
- [`docs/workflows/ai-loop/loop-safety-gates.md`](../../workflows/ai-loop/loop-safety-gates.md) Gate 1〜5・§6 既存正本との不整合防止一覧
- [`docs/workflows/ai-loop/00_concept.md`](../../workflows/ai-loop/00_concept.md) 収束ルール（対応ラウンド上限3）
- [`docs/workflows/ai-loop/adaptive-production-loop.md`](../../workflows/ai-loop/adaptive-production-loop.md) terminal state 表
- [`docs/workflows/ai-loop/flow-detect.md`](../../workflows/ai-loop/flow-detect.md) class=merge/no-merge 判定
...
- [`docs/working/TASK-0822/design-hotl-metrics.md`](./design-hotl-metrics.md) — reversal_rate との概念区別
