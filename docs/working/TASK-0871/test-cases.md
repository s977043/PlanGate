...
| AC-9 | 正本一覧に同じ責務の複数正本なし（ai-dev / ai-loop アーキ文書に限定。#866 は対象外） | TC-09 |
| AC-10 | command / ai-loop-cycle / 00_concept / six-stage / adaptive / Core Contract の参照整合 | TC-10, TC-11, TC-12 |
...
| TC-04 | S3 完了 | 正本の C-3/C-3' 経路節を確認。working-context.md / decision-table.md の記述と突合 | C-3' = eligible run の標準自動経路、Human C-3 = escalate 経路、判定主体（arbiter / Human）が 3 文書間で矛盾なし | 目視（独立レビューと重複可） |
...
| TC-09 | S4〜S6 完了 | ai-dev / ai-loop アーキ・責務定義文書を列挙し、責務・terminal state・decision table の定義（参照でない実定義）の所在を確認 | 各定義の実定義が 1 文書のみ。他は参照。#866 領域（skills 正本）は判定対象外と明記 | 目視（表で evidence 化） |
...
| TC-12 | S8 完了 | plan 付録 B の用語監査コマンド全件実行 | 旧定義・矛盾表現の残 = 0、または残す各件に採否理由が evidence にある | 機械 + evidence |
