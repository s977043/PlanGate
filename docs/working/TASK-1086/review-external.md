...
|---|---|---|---|
| R-001 | open | — | major。付け替えでは openai.yaml 欠落を検出できない（silently skip）/「即 FAIL」「CI が赤」は不成立。presence 検査の新設が必要 |
| R-002 | open | — | major。drift 4 件の向き未判定。`plan-review-gate` は `.codex` 側が新しく 36 行多い。untrack 前の reconcile 手順が要る |
| R-003 | open | — | minor。§4「全 48 ファイル」の表網羅率は 39/48。9 件を独立確認したが破壊なし＝結論は維持 |
| R-004 | open | — | minor。install 時に `openai.yaml` は target で再生成。S-1 の実害は marketplace 経路に限定 |
| R-005 | open | — | info。既定 target は既に 8 violations。付け替え前後の baseline を併記すべき |
