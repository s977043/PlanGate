...
| `PLANGATE_HOOK_STRICT=1 PLANGATE_HOOK_FILES="<10 fixture>" sh scripts/hooks/check-metrics-privacy.sh` | **PASS（rc 0・10 files checked）** |
| `npx markdownlint-cli2 <変更 2 doc>` | **0 issues** |

**変異注入による検出力の実証**（実装に欠陥を注入 → 該当テストが FAIL → 復元）:

| 注入した欠陥 | FAIL したテスト |
|-------------|----------------|
| M1: `MERGE_READY_CANDIDATE` を `MERGE_READY` に丸める | TC-57 負側 / TC-59 |
| M2: PR 未解決で `repair_rounds=0` に倒す | TC-64（2 件） |
| M3: `quality_metrics` に corpus 集計値を入れる | TC-60 |
| M4: `routing_decisions` 未供給を `[]` で埋める | TC-50 / TC-52 / round-trip |
| M5: `cause_hypothesis` を自動生成 | TC-18 |
| M6: harness drift を警告へ降格 | TC-31（3 subTest） |
| M7: 非終端 run でも EV を発行 | TC-57 負側 / TC-59 |
| M8: decision-only 以外の c3-prime NG を素通り | TC-40 |
| M9: 非 AUTO_APPROVED の後段束縛再検証を削除 | TC-40（2 件） |
| M10: legacy c3.json でも EV を発行 | legacy 拒否 TC |
| M11: `run_count` を record 件数にする | TC-54 |
| M12: decision 欠落を無言で捨てる | TC-44 |
| M13: `skipped[].file` を絶対パスにする | TC-44 |
| M14: invalid run meta を legacy に混ぜる | TC-44（分離） |
| M15: `round_index` の型検査を外す | TC-44 |
| M16: 入力の禁止キー検査を無効化 | TC-19 |
| M17: 絶対 `--evidence-ref` を許容 | **当初 survive** → 入力側 reject の識別子を assert に追加して kill（TC-20） |
| M18: URL 還元を無効化 | TC-51 |
| M19: 絶対パス還元を無効化 | TC-65 |
| M20: `--out` 拡張子検査を外す | TC-21 |
| M21: 出力側 privacy 最終検査を除去 | **当初 survive** → backstop の単体検査を追加して kill |
| M22: `blocked_by` 未注入を非 BLOCKED にする | TC-55 |
| M23: 2 件でも candidate を作る | TC-33 |
| M24: harness 混在でも baseline を決める | TC-26 |
| M25: improvement TASK に迂回フラグを足す | TC-27 |
| M26: rollback で `status` を維持 | TC-35 |
| M27: paired replay の disjoint 検査を外す | TC-34 |
| M28: `improvement_refs` の形式検査を外す | privacy reject TC |
| M29: `candidate_id` を run_id 順依存にする | 決定論 TC |

**plan / todo / test-cases と実装が食い違った点（実測を優先し契約 doc を是正）**:

1. **`harness_version` の注入経路**（契約 §2 #10 は「注入・fail-closed」だが §3-2 は
   「注入値 5 つで全数」と書いており両立しない）→ **§2 を優先**し
   `--harness-version` / `--harness-version-end` を追加。§3-2 を「run コンテキスト 5 +
   harness / 任意フィールド」の 2 表に是正。T-11 の `_args()` に注入を追加した。
2. **`--pr-number` は解決経路ではなく cross-check**。受理器は `kind=merge_ready` の
   `record.pr_number` からしか PR を再解決できないため、注入値だけで `repair_rounds` を
   実値化すると受理側が再計算照合できず（rc=1）、生成側の自己申告を信頼する構造になる。
3. **`quality_metrics` は `repair_rounds` に従属**するため `BLOCKED` の `unavailable` は
   **7 件ではなく 8 件**（test-cases.md fixture 表の「(a)3 + (b)4 = 7」は
   `quality_metrics` の従属を数えていない）。契約 §5 / §5-1 を是正。
4. **`ci_outcomes` に `kind=state` の `reasons` を混ぜない**（TC-14 が
   `len(ci_outcomes) == len(check_summary)` を assert するため）。`reasons` は
   `observation` へ回す。契約 §2 #12 を是正。
5. **§6-5 の解釈だけでは束縛検証に穴が空く**。`c3prime_verify` は
   `decision != AUTO_APPROVED` で return するため後段（`plan_hash` / `artifact_hashes` /
   `plan_package_hash` / reviewer snapshot）が未実行になる。producer 側で
   `c3_contract` の同一プリミティブを import して再検証する（変異注入 M9 で実証）。
6. **入力ソース allowlist の範囲**: c3-prime 再検証が Plan Package 6 artifact を open
   するため、ソース 1 を「`task_dir` 配下」と読み替える必要がある（契約 §3-1 に明記）。
7. **`ta` 番号**: plan / todo / test-cases の `ta-59` は #976 で使用済み。**`ta-60`** を使用
   （`git ls-tree origin/main --name-only tests/extras/` で再実測）。
