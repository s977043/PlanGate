...
### V-3 REJECT を受けて追加した受入基準

外部敵対レビュー（`review-external.md`）が critical 1 / major 3 を検出したため、
以下を追加する。

- **AC-7**（V-3 R-001）: 承認トークンの置き場は `approvals/` **だけではない**。
  保護ディレクトリを**集合**として扱い、いずれの配下でもファイル名 glob 崩しを
  block する。
- **AC-8**（V-3 R-002）: **brace expansion** を封鎖する。brace は
  **存在しないファイルを新規作成できる**ため、上書きしかできない glob より
  危険であり、残存クラス化ではなく封鎖を選ぶ。
- **AC-9**（V-3 R-004）: PreToolUse は全 Bash 実行のたびに走るため、
  **glob 語数に対して実行時間が劣化しない**こと（fork を作らない）。
- **AC-10**（V-3 R-003）: 保護ディレクトリ配下でも、**実際に保護されている
  ファイル種別以外は block しない**。また「保護名に一致しうる」だけの広い語は
  block しない（幅で判定する）。

### 追加の Out of scope（V-3 で明確化）

- `_has_write_intent` 側の既存ギャップ（`rm` / `chmod` / `gzip` / `touch`。V-3 R-010）
- 読み方向（source 位置の引数）を block 対象から外す**方向判定**
  （既存設計。V-3 R-003 #5/#11 の棄却根拠がここに依存する）
