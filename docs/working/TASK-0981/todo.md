...
> 基点: main `7de7baa` / 成果物は**文書のみ**（コードを 1 行も変更しない）
...
- [ ] **T-01**: 基点更新と棚卸し 2 表の再実測（plan Step 1 / U-7 の裏取り含む）
  - main を `7de7baa` として、pbi-input のギャップ 12 項目表と `EC-1`〜`EC-10` 表の根拠を**再走査**する。少なくとも以下を実測して `status.md` に記録:
...
- [ ] **T-02**: ADR を新規作成し、誤読防止の冒頭 1 文を置く（plan Step 2 / U-1）
  ...
  - 🚩 **チェックポイント**: 冒頭 1 文が本文の**最初の段落**に存在（TC-02）+ ADR-001 と同じ節見出しが揃う（TC-01）
...
- [ ] **T-07**: 現状維持 / 補強の 4 判断を ADR に記録（plan Step 7 / D-6・D-7・D-8・D-9）
  - **D-6（legacy 経路 / ギャップ #5）**: PR2 で `bin/plangate` の legacy 経路にある「`plan_hash` 不在なら hash 突合を無言 skip」という **fail-open 1 点のみ**を BLOCK 化する patch を提示（`bin/plangate` は HO → **AI は patch 提示まで**）。evidence marker 再検証・`artifact_hashes` 照合の全面移植は**行わない**（既存 TASK の `c3.json` を一斉 invalid 化し後方互換を壊すため）
