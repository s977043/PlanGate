...
| 2 | Plan Package 全体の hash 固定 | **既存で満たす** | `scripts/ai-loop/c3_contract.py:71-74`（`canonical_hash()` = `sort_keys` + 最小区切りの正規化 JSON の sha256）/ `plan_package.py:141-146`（`artifact_hashes` 6 要素 → `plan_package_hash`）/ 受理側再計算照合 `c3prime_verify.py:131-143`（`artifact_hashes` 全数 + `plan_package_hash` 再計算一致）/ 契約 §2 `c3-prime-contract.md:46-47` |
...
| 5 | Executor 実行直前の再検証 | **既存で満たす（c3-prime 経路）／一部満たす（legacy 経路）** — 経路で強度が非対称。「Executor が誰か」はどちらの経路でも検証対象外（→ #8/#9） | **c3-prime 経路（強い）**: `bin/plangate:2047-2066`（exec 時に `git rev-parse HEAD` を解決 → 解決不能かつ c3-prime なら即 BLOCK → `_plangate_c3_dispatch "$work_dir" "$_exec_head"`）/ 共有ヘルパ `bin/plangate:885-912`（検証器不在時も `approval_kind` 有りなら NG=fail-closed）/ `c3prime_verify.py:46-166`（構造 allowlist・task_id↔task_dir 束縛・evidence 再検証・decision・source_sha・plan_hash・artifact_hashes・plan_package_hash・reviewer 三つ組・evidence_ref 独立性）/ ai-loop Delivery 側も同じ verifier を通す `delivery.py:498-506`（`verify_c3()`）+ `delivery.py:521-529`（`--expected-sha` を**必須化**）。**legacy 経路（弱い）**: `c3prime_verify.py:62-67` は `approval_kind` キー不在時に **exit 10 で legacy 委譲**し、`bin/plangate:2070-2076`（`grep '"c3_status"'` + sed で `APPROVED` 判定）と `2078-2106`（`plan.md` 単体 hash 突合）**のみ**が走る = **evidence marker 再検証なし / `artifact_hashes` なし / `source_sha`↔HEAD 束縛なし**。さらに `bin/plangate:2092` の `if [ -n "$recorded_hash" ]` により **c3.json に `plan_hash` が無ければ hash 突合そのものが無言で skip**（`cmd_validate` は `bin/plangate:1024` で `[WARN] plan_hash not found in c3.json` を出すが **exec は無言**） |
| 6 | `allowed_paths` 抽出・検証 | **既存で満たす**（**抽出・宣言検証は事前／逸脱検出は事後**。書込時の事前強制は `allowed_paths` 由来では無い） | **事前（抽出・宣言の妥当性）**: 単一実装 **`extract_allowed_paths()`** `plan_package.py:170-185`（`## Files / Components to Touch` 節 + `` `path/like` `` 正規表現。0 件は例外でなく空リスト）/ Collector は再実装せず再利用 `collector.py:821-827`、0 件は `escalation_flags` へ `allowed_paths_empty` を積む `collector.py:132`・`collector.py:1057-1059`・`collector.py:1075` / LoopSpec 派生側は 0 件を fail-closed `plan_package.py:211-214`。**事後（逸脱の検出）**: `delivery.py:263-269` は Collector snapshot の `changed_files` と突合 → `EXEC_RETURN`、`arbiter.py:362-374` の glob 照合も同様に**変更後**の判定。すなわち **executor が書いた後に検出**する層であり、exec 前に書込先を制限するものではない。**書込時の事前 block は EH-6（`scripts/hooks/check-forbidden-files.sh`）が存在するが、その scope 源は Orchestrator Mode の子 PBI YAML（`docs/working/PBI-*/children/` の `allowed_files` / `forbidden_files`）であって `plan.md` の `allowed_paths` ではない**（`check-forbidden-files.sh:10-17`）→ 「`allowed_paths` に基づく事前強制」は不在 |
...
| 8 | Planner / Executor 分離 | **未対応**（暗黙ですらない） | exec が記録する唯一の主体情報は `bin/plangate:2037`（`agent="${PLANGATE_IMPL_AGENT:-codex}"`）→ `bin/plangate:2110-2113`（`session_started` の `detail":"agent=$agent"`）。これは**ツール種別**（`codex` / `opencode` / `cursor`）であり、session ID でも Principal でもない。**schema 側の正確な状況（River Review MJ-3）**: `run-event.schema.json` は `agent`（`:48-51`「Agent or tool that produced the event」）と `by`（`:52-55`「Human or agent identifier (used for gate events)」）を**既に持っている**。ただし `bin/plangate` の `plangate_append_ndjson` 呼び出し 3 箇所（`:1279` / `:2005` / `:2112`）は**いずれもこの 2 プロパティを使っていない**（すべて `detail` 文字列に詰めている）。したがって「**`session_id` という名称のキーは `additionalProperties: false`（`:77`）により schema 違反／`agent`・`by` は既存プロパティで writer 未使用**」が正確な記述である。Plan を作った主体を記録するフィールドは c3-prime record 側にも無い（`c3_contract.py:44-49`） |
| 9 | `ExecutionStarted` から approval への参照 | **未対応** | `session_started` イベントは `ts` / `task_id` / `phase` / `event` / `detail` のみ（`bin/plangate:2110-2113`）。`run-event.schema.json` には `plan_hash` プロパティが**存在する**が（`:56-60`、説明は「gate event 時の plan.md hash」）、exec の `session_started` 書き込みでは**使われていない**。`approval_ref` / `approval_event_id` 相当のプロパティのみが真に不在。→ **schema 無変更で刻める既存プロパティが `plan_hash` / `agent` / `by` の 3 つある**（PR2 の最小差分候補。HO 変更は不可避ではない）。現状は「実行開始 → どの承認に基づくか」の因果が record から追跡できない |
| 10 | Plan revision / approval revoke | **未対応** | `plan_version` / `plan_revision` を持つ実装・schema は**存在しない**（`grep -rn "plan_version\|plan_revision" scripts/ schemas/ bin/` → **0 件**。`schema_version` は artifact schema のメタで別概念: `schemas/plan.schema.json:7,17` 等）。承認後の Plan 変更は「hash 不一致で**拒否**」されるだけで（`c3prime_verify.py:127-128`）、旧 approval を **revoke 済みとして記録する経路が無い**。`PlanApprovalRevoked` / `PlanRevisionCreated` 相当のイベントも `run-event.schema.json:25-46` の `event` enum に不在 |
| 11 | Resume / Retry 時の再検証 | **一部満たす**（ai-loop Delivery 段のみ / exec 再開は非対応） | **満たす側**: `delivery.py:521-536` は毎回 `verify_c3(task_dir, --expected-sha)` を通してから assess するため、Delivery ループの再実行では Plan Package・approval・HEAD が再検証される。二重作用の抑止は intent / receipt の 2 段書き込み（`docs/workflows/ai-loop/delivery-state-machine.md:89`、`delivery.py:404-420`）と `executor.py` の pre-check（**設計意図の記述は `executor.py:26-45` の module docstring。実装本体は同ファイル内の別位置**）。**未対応側**: `bin/plangate resume`（`bin/plangate:1343-1364`）は `current-state.md` を `cat` して `cmd_status` の 2 行を grep するだけで、**plan hash / approval / HEAD の再検証を一切行わない**。exec セッションの再開に契約上の再検証点が無い |
| 12 | plan version 番号 | **hash で代替済み**（新設不要 — 詳細は「Notes from Refinement / `plan_version` 要否の結論」） | #1〜#4 のとおり、実行同一性は `plan_hash`（`plan.md` 単体）と `plan_package_hash`（6 要素の正規化集合）の 2 段で既に閉じている。承認との束縛・stale 検出・受理側再検証がすべて hash を正としており（`c3prime_verify.py:124-143`）、番号は同一性判定に一切使われていない。番号を追加すると「番号は同じだが内容が違う」状態を作れてしまい**二重正本**になる |

### 12 項目の集計

| 判定 | 件数 | 項目 |
|---|---|---|
| 既存で満たす | **5** | #1 Plan 本文 hash / #2 Plan Package hash / #3 C-1・C-2 束縛 / #4 C-3' approval 束縛 / #6 `allowed_paths` の抽出・宣言検証 |
| 一部満たす | **3** | **#5 実行直前再検証（c3-prime は満たす／legacy 経路は `c3_status` + 任意 `plan_hash` のみ）** / #7 maker/checker 分離（LoopSpec 派生時のみ・record 未刻印）/ #11 Resume 再検証（Delivery のみ・exec 再開なし） |
...
> **したがって PR2 以降の追加実装対象は #5（legacy 経路）・#7・#8・#9・#10・#11 の 6 点に限定される**（#1〜#4・#6 は再実装しない = 二重正本の回避）。
>
> **#5 を「既存で満たす」と丸めない理由（River Review MJ-1）**: PlanGate 本番フローの大多数の TASK（**本 TASK-0981 自身を含む**）は人間 C-3 = **legacy 経路**で回る。#5 を一律「既存で満たす」と書くと、AC-1「既存で満たす項目に PR2 以降の作業を割り当てない」を額面どおり適用した結果、**legacy 経路の穴が恒久的にスコープ外へ落ちる**。強度が経路で非対称であることを判定に残し、PR2 のスコープに明示的に載せる。

---

## 裏取り結果 2: `ExecutionStarted` 成立条件 10 項目の充足状況（**c3-prime 経路の検証状況**）

issue コメント §3.3 の 10 条件について、現行実装で「実行開始前に機械検証されているか」を実コードで確認した。

> **本表の適用範囲（River Review MJ-1）**: 以下は **`approval_kind == "c3-prime"` の経路**の検証状況である。**legacy 経路（人間 C-3 = `approval_kind` キー不在）では EC-2 / EC-5 / EC-6 / EC-7 は検証されない**（`c3prime_verify.py:62-67` が exit 10 で委譲し、`bin/plangate:2070-2106` の `c3_status` + 任意 `plan_hash` のみが走る）。ギャップ #5 を参照。
>
> **番号空間の分離（River Review mn-4）**: ギャップ表の `#1`〜`#12` と混同しないよう、本表の条件は **`EC-1`〜`EC-10`**（Execution Condition）で参照する。

| # | 条件（issue コメント §3.3） | 充足 | 根拠と注記 |
|---|---|---|---|
| EC-1 | Plan Package 6 要素が存在し非空 | **部分** | 存在（`is_file`）は受理側で検証 `c3prime_verify.py:134-137`。**0 byte 非空の明示検査は生成側のみ**（`plan_package.py:50-60` `check_presence()`）。受理側で 0 byte が通らないのは「hash 一致 + 生成時に presence を通っている」ことの帰結であり、受理側単独の不変条件としては書かれていない（PR1 の ADR で「受理側 presence の意味範囲」を明記すべき点 → U-6） |
| EC-2 | C-1 / C-2 evidence が現行 `plan.md` の hash と一致 | **充足** | `c3prime_verify.py:100-102` が受理側で `plan_package.check_evidence()` を再実行（`plan_package.py:100-129`）。marker 内 plan hash と現 `plan.md` sha256 の照合のみ・mtime 不使用 |
| EC-3 | c3-prime decision が `AUTO_APPROVED` | **充足** | `c3prime_verify.py:104-109`（3 値 allowlist → `AUTO_APPROVED` 以外は fail） |
| EC-4 | c3-prime の `plan_hash` が現行 `plan.md` と一致 | **充足** | `c3prime_verify.py:124-128` |
| EC-5 | `artifact_hashes` と現行 6 artifact が一致 | **充足** | `c3prime_verify.py:131-139`（キー集合一致 + 全数 sha256 照合・不一致名を明示） |
| EC-6 | `plan_package_hash` が再計算値と一致 | **充足** | `c3prime_verify.py:142-143`（`c3_contract.canonical_hash(ah)` `c3_contract.py:71-74` と突合） |
| EC-7 | `source_sha` が実行時 HEAD と一致 | **充足**（exec 経路のみ） | `bin/plangate:2051-2066`（HEAD 解決 → 不能かつ c3-prime なら BLOCK → verifier へ `expected_sha` 注入）+ `c3prime_verify.py:111-121`。**静的 `validate` は `expected_sha` を渡さないため形式チェックのみ**（契約 §4 `c3-prime-contract.md:84` の明記どおり）。SHA 一致の強制点は exec |
| EC-8 | Executor が参照する Plan hash が approval の Plan hash と一致 | **未対応** | Executor 側に「参照した Plan」を宣言するフィールドが存在しない。同一性は「同じ `task_dir` を見ている」という**暗黙の前提**に依存する。`task_id` の task_dir 束縛（`c3prime_verify.py:82-84`）は別 TASK の record 流用は防ぐが、**Executor の参照宣言と approval の照合**は行っていない |
| EC-9 | Executor session が空でない | **未対応** | 記録されるのは tool 種別のみ（`bin/plangate:2037,2110-2113`）。session ID の概念・非空検証ともに不在。**ただし schema 側は完全な白紙ではない**（MJ-3）: `run-event.schema.json` は `agent`（`:48-51`）/ `by`（`:52-55`）を既に持ち writer が未使用。`session_id` という**名称**の新キーのみが `additionalProperties: false`（`:77`）に抵触する |
| EC-10 | prohibited action に merge が含まれ、AI merge authority が付与されていない | **部分（実質は強い / 宣言は不在）** | **実質**: `gh_exec.py:29-46`（`gh pr merge` は allowlist の補集合として自動的に禁止・`graphql` は allowlist に載せない・`NO MERGE BY AI` 明記）+ `check_exec_boundary.py:1-55`（実行系トークンを AST で機械強制・`gh_exec.py` 以外は `subprocess` 不可）+ `executor.py:59-60`（merge / approve / close を組み立てない）。**不在**: record 側に `prohibited_actions` / `allowed_actions` の**宣言フィールドが無い**（`c3_contract.py:44-49`）。また `gh_exec.py:39-46` が自認するとおり in-process allowlist は**別プロセスからの `gh pr merge` を塞がない** |

### 10 条件の集計

| 判定 | 件数 | 条件 |
|---|---|---|
| 充足 | **6** | EC-2 evidence 束縛 / EC-3 decision / EC-4 plan_hash / EC-5 artifact_hashes / EC-6 plan_package_hash / EC-7 source_sha（exec 経路） |
| 部分 | **2** | EC-1 presence（0 byte 検査は生成側のみ）/ EC-10 merge 禁止（実装は強いが宣言フィールド不在・別プロセス経路は塞がらない） |
| 未対応 | **2** | EC-8 Executor の Plan 参照照合 / EC-9 Executor session |

> すなわち `ExecutionStarted` の 10 条件のうち **Plan 同一性に関する 6 条件は既に（c3-prime 経路の）exec preflight で機械強制されており、不足しているのは「実行主体（EC-8・EC-9）」と「宣言の明文化（EC-1 の受理側 presence 範囲・EC-10 の prohibited action 宣言）」の 4 点**である。
>
> **EC-1 / EC-10 の帰属（River Review mn-4）**: この 2 件は PR2/PR3 の実装差分ではなく **PR1 の ADR で「現状維持 + 明記」か「補強」かを決定する**（それぞれ U-6 / U-3）。PR2 以降のスコープ表に現れないのはそのため。
...
| S-8 | **legacy 経路（人間 C-3）の preflight 強度をどう扱うかを決定**（強化する / 明示的に非対象とし理由を記録する）。ギャップ #5 の分割判定に対応（River Review MJ-1） | ADR の決定事項 + PR2 スコープへの反映 |
| S-9 | **`c3-prime-contract.md` §8 への但し書きの要否を判断**（§8「additive な任意フィールド追加は本ファイルの改版のみでよい」`c3-prime-contract.md:137` は、`^_` 注釈キー以外の record フィールドでは実態と緊張関係にある — `RECORD_ALLOWED_KEYS` と schema の同時更新が要る。River Review inf-3） | ADR の記述 + 必要なら契約への追記案 |
...
#981 PR1: 棚卸し・ADR・正本整理      ← 本 PBI
  ↓
#981 PR2: Executor handoff / exec preflight 拡張（ギャップ #5 legacy 経路・#7・#8・#9）
  ↓
...
| **PR2（追加 / River Review MJ-1）: legacy 経路の preflight 強化、または「legacy は明示的に非対象」という判断の記録** | ギャップ #5 のとおり、legacy 経路（人間 C-3 = **本 TASK-0981 自身を含む大多数の TASK**）の exec preflight は `c3_status` grep + 任意 `plan_hash` のみで、`plan_hash` 不在時は**無言で skip**（`bin/plangate:2092`）。PR2 が c3-prime 分岐だけを拡張すると、この穴が恒久的に残る。**強化する場合の変更先は `bin/plangate` = HO 対象**のため、Human 適用を伴う patch 運用になる点も併せて S-8 で判断する |
...
|---|---|---|
| **AC-1** | 追加実装対象が「未対応差分」だけに限定されている | 要件対応表（S-3）で 12 項目それぞれに「既存で満たす / 一部満たす / 未対応」と根拠ファイル:行が付き、**「既存で満たす」5 項目（#1・#2・#3・#4・#6）に PR2 以降の作業が 1 件も割り当てられていない**ことを表上で確認できる。**かつ「一部満たす」3 項目（#5・#7・#11）については、満たしている側と満たしていない側が経路・条件つきで書き分けられ、満たしていない側が PR2/PR3 に割り当てられている**（River Review MJ-1: 「一部」を「既存で満たす」に丸めると legacy 経路が構造的にスコープ外へ落ちる） |
...
| **AC-3** | 新規 schema 追加の必要性が説明されている | ADR に「既存 schema（`c3-prime.schema.json` / `run-event.schema.json`）の additive 拡張で足りるか、sidecar schema が要るか」の評価が残り、**要る場合はその理由**（既存 schema の `additionalProperties: false` 制約・HO 対象パスであること等）と**要らない場合の代替**が併記されている。**評価には HO 変更を伴わない 3 経路（`^_` 注釈キー / `run-event.schema.json` の既存未使用プロパティ `plan_hash`・`agent`・`by` / sidecar）を必ず候補として検討した記録を含む**（River Review MJ-2・MJ-3: これらを見落とすと不要な HO patch を前提に計画してしまう） |
...
`plan_revision` を**任意で**導入する場合の条件（plan で確定）:

- **(a) 格納形式は 2 択しかない（River Review mn-1）**。「必須キーにしない」だけでは**不十分**で、受理器 `c3prime_verify.py:73` + `RECORD_ALLOWED_KEYS`（`c3_contract.py:44-51`）は**未知キーを任意であっても reject** するため、素の `plan_revision` を載せた record は **fail する**。成立するのは **(i) `_plan_revision` として `^_` 注釈キーで持つ**（schema/受理器とも無変更）、**(ii) `RECORD_OPTIONAL_KEYS` + schema に追加する**（HO 変更を伴う）の二択
- **(b)** 受理器 `c3prime_verify.py` の判定分岐に一切使わない
- **(c)** 人間が「これは 3 回目の計画版だ」と読むための表示専用であることを ADR に明記する
...
| ② `approvals/c3.json` へ additive な参照追加 | **コストは 2 段階（River Review MJ-2 で是正）**。**(a) `^_` prefix の string 注釈キー（例 `_execution_ref`）は schema・受理器とも無変更で追加可能** — 受理器 `c3prime_verify.py:73` は `if k not in ALLOWED_KEYS and not k.startswith("_")` で `_` 始まりを allowlist 検査から除外し（型検査は `:95-97` の string のみ）、`schemas/c3-prime.schema.json:113-114` も `patternProperties: {"^_": {"type":"string"}}` を持ち `additionalProperties: false`（`:119`）の例外になっている。契約 `c3-prime-contract.md:55` も明記。**(b) 型付きの第一級フィールド**は `RECORD_OPTIONAL_KEYS`（`c3_contract.py:50-51`）+ schema の**同時変更が必須**で、`schemas/*.schema.json` は **Hardening Override 対象**（AI 直接編集不可）| **条件付き**。**HO コストを理由に棄却はできない**（(a) の経路がある）。棄却するなら理由は設計上の帰属 — execution 側の情報（executor session 等）は承認 record ではなく**実行 record** に属し、承認 record は「承認時点の不変スナップショット」であるべきで、1 承認 : N 実行の関係を表現できない |
...
- `schemas/run-event.schema.json:77` は `additionalProperties: false`。`session_id` / `approval_ref` という**名称の新キー**を書くと **schema 違反**になる
- 同 schema `:25-46` の `event` enum に `ExecutionRequested` / `ExecutionStarted` / `PlanApprovalRevoked` 等は不在（`session_started` / `exec_started` は存在）
- **schema 無変更で使える既存プロパティが 3 つある**（River Review MJ-3）: `plan_hash`（`:56-60`）/ `agent`（`:48-51`「Agent or tool that produced the event」）/ `by`（`:52-55`「Human or agent identifier (used for gate events)」）。**いずれも `bin/plangate` の `plangate_append_ndjson` 呼び出し 3 箇所（`:1279` / `:2005` / `:2112`）で未使用**（すべて `detail` 文字列に詰めている）→ **PR2 の最小差分候補**
- **ただし CI の schema 強制は `run.ndjson` に及んでいない**（River Review inf-2）: `scripts/schema_mapping.py:36` の対応キーは basename `run-event.json` であり、`scripts/validate-schemas.py:80` は `base.rglob("*.json")` のみを収集する。**拡張子が `.ndjson` の `run.ndjson` はどちらにも掛からない** → 「schema 違反になる」は**契約層の記述としては正しいが、CI による機械強制は無い**。U-2 の比較では「enforcement の有無」を明示的な判断軸に含める
...
- 定量（PR1 想定）: 変更ファイル数 = ADR 1 + working context 6〜7 + `c3-prime-contract.md` 追記 0〜1 → **7〜9 ファイル**（下限 = 1+6+0 = 7）→ **high-risk 帯**（6-15）/ AC 6 個 → **high-risk 帯**（6-10）。**定量・定性の両軸とも high-risk に落ちる**（River Review mn-5 で下限の算術誤りを是正。結論は変わらず強化される）
...
| **`check-plan-hash.sh` に責務を寄せたくなる**（EH-3 は既に plan_hash を見ているため「ついでに」の誘因がある） | hook は PreToolUse の補助防衛であり、環境変数で挙動が変わる（`PLANGATE_BYPASS_HOOK` = 常時 exit 0: `check-plan-hash.sh:43-47` / `PLANGATE_HOOK_STRICT` = 違反時のみ exit 1、既定は WARN で exit 0: `check-plan-hash.sh:362`）。**既定では違反しても通る**。実行許可の正本を置くと**バイパス可能な承認**になる | Non-goals に明記済み。ADR で「実行許可の正本 = exec preflight strict verifier / EH-3 = 補助防衛」を層として固定し、PR2/PR3 の変更対象から `scripts/hooks/` を外す |
...
- **U-2**: **execution reference の物理的な置き場**。sidecar（`docs/working/TASK-XXXX/execution/plan-contract.json`）/ `run.ndjson` の additive イベント / `approvals/c3.json` の `^_` 注釈キー / これらの併用のいずれか。判断軸は (a) HO 接触の有無、(b) 承認 record の不変性を壊さないか、(c) 1 承認 : N 実行の関係を表現できるか、**(d) enforcement の有無**（River Review inf-2: sidecar が `*.json` として `validate-schemas.py` の rglob に掛かるのに対し、`run.ndjson` は拡張子が外れており CI 検証されない。「契約に書いてあるか」と「機械強制されるか」を混同しない）
...
- **U-7**: **`derive_loopspec()` の maker/checker を record に刻むか**（ギャップ #7）。型付きフィールドとして刻むと `c3-prime.schema.json`（HO 対象）+ `RECORD_OPTIONAL_KEYS` の同時変更が必要（`^_` 注釈キーなら無変更 — MJ-2 参照）。`derived_loopspec_hash` という任意フィールドが既にある（`c3-prime-contract.md:54`）ため、LoopSpec 側に閉じる案も成立しうる
- **U-8**（River Review mn-2 で追加）: **evidence stale 判定の適用範囲**。現行の C-1/C-2 marker 照合は **`plan.md` 単体の sha256 のみ**に束縛されている（`plan_package.py:109` `current_plan_hash = _sha256_of(plan)` / `:125-128`）。したがって `todo.md` / `test-cases.md` 等の変更は `artifact_hashes`（record 側）でしか検出されず、**record を再生成すれば C-1/C-2 を再実行しないまま marker が有効なまま残る**。stale の束縛先を `plan.md` 単体のままにするか `plan_package_hash` へ広げるかは、**PR3 の revision 契約の前提**になるため PR1 の ADR で方向を決める
