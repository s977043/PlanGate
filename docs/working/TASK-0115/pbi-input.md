...
- [`responsibility-classes.md`](../../../.claude/rules/responsibility-classes.md) または `.claude/rules/` 配下に「Bash 連結コマンド時の error guard」セクション追加 (R-008 反映: 「既存ルール対応」セクション直前に配置で論理構造維持)
  - `&&` 連結 or `set -e` 必須
  - `git push` 前の `git rev-parse --abbrev-ref HEAD` で current branch verify
  - **`main` は直接 commit / push 禁止** ([`project-rules.md`](../../ai/project-rules.md) L66 と一致 / R-001 反映)
...
- AC-3 (R-001 反映): AI 運用 4 原則 ([`CLAUDE.md`](../../../CLAUDE.md) `<law>`) との階層関係明示 (本 rule は第 1 原則の運用解釈)、**`main` は禁止 / 他 protected は明示確認** という 2 段階構造を明示
