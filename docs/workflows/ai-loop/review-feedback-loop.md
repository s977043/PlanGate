...
### 2-7. Remember / Optimize 分離との対応

> 対応 issue: [#709](https://github.com/s977043/plangate/issues/709)

本フロー（§2-1〜§2-6）は、6 層自己改善ループ（正本:
[`adaptive-production-loop.md`](./adaptive-production-loop.md)。
Generate → Evaluate → Remember → Schedule → Optimize → Recurse）における
**Remember 層と Optimize 層の PoC 実体**である。6 層モデル自体の定義・
L0〜L5 との対応は同ドキュメントを正本とし、本節では再定義しない
（上位概念は adaptive-production-loop.md、下位実装は本ドキュメント §2）。

| 層       | 本フローでの対応                                                     | 責務                                                                           |
| -------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Remember | §2-1（収集）+ §2-2（分類）                                           | 指摘・分類・severity を事実として保存する。まだ何も改善しない                  |
| Optimize | §2-3（還元先判定）+ §2-4（反映）+ §2-5（事前適用）+ §2-6（効果測定） | Remember された記録を根拠に、次回の gate / skill / suppression（§5）を更新する |

分離の狙いは以下の 2 問題を同時に防ぐことである。

- **記録するだけで改善しない**問題: Remember だけでは指摘が蓄積されるのみで
  次回に活きない。Optimize（§2-3〜§2-6）が「還元先判定 → 反映 → 事前適用 →
  効果測定」まで担うことで、記録が実際に gate/skill/suppression の更新へ
  変換されることを保証する。
- **根拠なく書き換える**問題: Optimize は Remember（R-NNN・分類・
  トレーサビリティ表）を根拠としてのみ還元先を書き換える。根拠のない
  場当たり的な観点追加・policy 変更は行わない。

Optimize が policy への還元を含む場合は §6 の安全制約（第0の承認境界 =
Human-owned 固定）にそのまま従う。Remember/Optimize の分離は承認境界を
変更しない（[`adaptive-production-loop.md`](./adaptive-production-loop.md) §6
と同一の不変条件）。
...
- [`docs/workflows/ai-loop/adaptive-production-loop.md`](./adaptive-production-loop.md) — 6 層自己改善ループの正本。本ドキュメント §2-7 の上位概念（issue #709）
