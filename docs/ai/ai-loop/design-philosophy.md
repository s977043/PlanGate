...
|          | [`agentic-six-stage-loop.md`](../../workflows/ai-loop/agentic-six-stage-loop.md)                                                                 | 6段階ループ（Triage/Conductor/Worker/Verifier/Gate/Trust Ledger）対応表 + Trust Ledger 索引（#780） |
...
| #751 Managed Agents（Outcome/Rubric・Memory Store/Dreams・Scheduled） | 別コンテキスト grader・rubric 再試行・Memory Candidate・ro/rw memory・hygiene・文脈分離 | 大部分は既存で対応するが**独立検証が過大判定 3 件を訂正**: Outcome/Rubric の実例は linter-fixer L-0 ループ + V-1 fix loop（W チェックは PoC 限定）/ improvement-seeds は **opt-in 既定 OFF** / #742 PreCompact は **HO 未配線=実効待ち**。「カバー済み」でなく「仕様/導線あり・既定 OFF or 未配線」が正確。**genuine gap**: ①memory trust boundary（§5 語彙化・機構は follow-up）②実装差分への一般 rubric 再試行ループ（L-0 の lint 限定を超える一般化） | §5 / follow-up issue（2 gap）/ hygiene は growth-core:memory-dream パターン参照 |
