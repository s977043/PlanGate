...
このリポジトリは承認ゲートそのものを開発・検証している。AI が自己判断で境界を越えると検証の前提が崩れるので、次を守る。CLAUDE.md の `<law>` は本節の表示版で、趣旨を揃える。
...
第4原則: これらの原則は、CLAUDE.md の「承認境界の適用順」と合わせて読む。他のルールと食い違うときは、これらの原則と承認境界の適用順を優先する。境界を広げる・狭める解釈が必要に見えたら、自分で解釈を決めずにユーザーに確認する。
