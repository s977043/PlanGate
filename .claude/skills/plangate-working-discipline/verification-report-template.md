...
両者が食い違う報告は不正とする。整合は**正本である `outcome-contract.md` の定義に従って**
行う（本レポート側の判定を見直す。正本側を書き換えて辻褄を合わせない）。
...
- 検証コマンドは**計画時に実機で事前検証済み**のものを使う（PASS/FAIL 両方向を確認済み。
  環境依存 — BSD/GNU の sed の仕様差等 — でサイレントに空を返すコマンドは「機械検証」を偽装する）。
